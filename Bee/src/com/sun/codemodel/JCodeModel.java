 ...
 * given time (which can be often accesesd by the {@code owner()} method.)
 *
 * As such, when you generate Java code, most of the operation works in a
 * top-down fashion. For example, you create a class from {@link JCodeModel},
 * which gives you a {@link JDefinedClass}. Then you invoke a method on it to
 * generate a new method, which gives you {@link JMethod}, and so on.
 *
 * There are a few exceptions to this, most notably building
 * {@link JExpression}s, but generally you work with CodeModel in a top-down
 * fashion.
 *
...
public final class JCodeModel {
	...
	private final HashMap<String, JReferencedClass> refBClasses = new HashMap<String, JReferencedClass>();
	...
	/**
	 * Creates Java module instance and adds existing packages with classes to the
	 * Java module info. Used to initialize and build Java module instance with
	 * existing packages content.
	 * 
	 * @param name
	 *            The Name of Java module.
	 * @param requires
	 *            Requires directives to add.
	 * @throws IllegalStateException
	 *             when Java module instance was not initialized.
	 */
	...
	/**
	 * Adds existing packages with classes to the Java module info. Java module
	 * instance must exist before calling this method. Used to update Java module
	 * instance with existing packages content after it was prepared on client side.
	 * 
	 * @param requires
	 *            Requires directives to add.
	 * @throws IllegalStateException
	...
	public JClass ref(Class<?> clazz) {
		...
		if (jrc == null) {
			jrc = refBClasses.get(clazz.getName());
		}
		if (jrc == null) {
	...
	public JClass ref(BClass bclass, BProject project) {

		Iterator<Class<?>> ito = refClasses.keySet().iterator();
		while (ito.hasNext()) {
			Class<?> cls = ito.next();
			if (cls.getName().equals(bclass.getQualifiedName())) {
				return refClasses.get(cls);
			}
		}

		JReferencedClass cls = refBClasses.get(bclass.getQualifiedName());

		if (cls == null) {
			if (bclass.isArray()) {
				return new JArrayClass(this, ref((BClass) bclass.getArrayPressentClass(), project));
			} else {
				cls = new JReferencedModelClass(bclass, project);
				refBClasses.put(bclass.getQualifiedName(), cls);
			}
		}
	...
	public JClass refExist(String fullName) {
		Iterator<Class<?>> ito = refClasses.keySet().iterator();
		while (ito.hasNext()) {
			Class<?> cls = ito.next();
		...
		Iterator<String> ito1 = refBClasses.keySet().iterator();
		while (ito1.hasNext()) {
			String n = ito1.next();
			if (n.equals(fullName)) {
				return refBClasses.get(n);
			}
		}
		return null;
	}
	...
	public JClass wildcard() {
		if (wildcard == null)
			wildcard = JClass.wildcard(this, ref(Object.class));
		return wildcard;
	}
		...
		JClass parseTypeName() throws ClassNotFoundException {
			...
			if (s.charAt(idx) == '?') {
				...
				if (head.startsWith("extends")) {
					idx += 7;
					ws();
					JClass jclass = parseTypeName();
					return JClass.wildcard(jclass.owner(), jclass);
	 ...
	 * 
	 * <p>
	 * JReferencedClass is kept in a pool so that they are shared. There is one pool
	 * for each JCodeModel object.
	 * 
	 * <p>
	 * It is impossible to cache JReferencedClass globally only because there is the
	 * _package() method, which obtains the owner JPackage object, which is scoped
	 * to JCodeModel.
	 */
	public class JReferencedClass extends JClass implements JDeclaration {
		private final Class<?> _class;

		// just for bclass extends use
		JReferencedClass() {
	...
	public class JReferencedModelClass extends JReferencedClass {
		private final BClass _class;
		private BProject project;

		JReferencedModelClass(BClass _clazz, BProject project) {
		...
		public String name() {
			// if (_class.getLogicName().indexOf("$") > 0) {
			// return _class.getLogicName().substring(_class.getLogicName().indexOf("$") +
			// 1);
			// }
			return _class.getLogicName().replace('$', '.');
		}
		...
		public JClass outer() {
			String fullName = _class.getQualifiedName();
			if (fullName.indexOf('$') > 0) {
				String name = fullName.substring(0, fullName.lastIndexOf('$'));
				JReferencedClass bcls = refBClasses.get(name);
				if (bcls != null) {
					return bcls;
				}
				BClass bclass = CodecUtils.getClassFromJavaClass(project, name);
				if (bclass != null) {
					return ref(bclass, project);
				}
				return ref(name);
			}
			String parent = _class.getInnerParentClassName();
			if (parent != null) {
				return ref(parent);
		...
		public JClass _extends() {
			BValuable s = _class.getSuperClass();
			if (s == null) {
				return null;
			}

			BClass sp = s.getBClass();
			if (sp == null) {
				if (isInterface())
					return owner().ref(Object.class);
				return null;
			} else
				return ref(sp, project);
		}

		public Iterator<JClass> _implements() {

			final List<BValuable> interfaces = _class.getInterfaces();

			return new Iterator<JClass>() {
				private int idx = 0;

				public boolean hasNext() {
					return idx < interfaces.size();
				}

				public JClass next() {
					return JCodeModel.this.ref(interfaces.get(idx++).getBClass(), project);
				}
		...
		public boolean isStatic() {
			return Modifier.isStatic(_class.getModifier());

		}

		public boolean isAbstract() {
			return Modifier.isAbstract(_class.getModifier());
		}
	...
	/**
	 * Conversion from primitive type {@link Class} (such as {@link Integer#TYPE} to
	 * its boxed type (such as {@code Integer.class})
	 */

...
import com.sun.codemodel.JCodeModel.JReferencedClass;
...
public final class JFormatter {
	...
	private boolean simplePrinting = false;
	...
	public void setSimplePrinting() {
		this.simplePrinting = true;
	}

	public boolean isSimplePrinting() {
		return simplePrinting;
	}
	 ...
	 *
	 * <p>
	 * In the collecting mode we use this information to decide what types to import
	 * and what not to.
	 */
	public JFormatter t(JClass type) {
		...
		switch (mode) {
		case PRINTING:
			// many of the JTypes in this list are either primitive or belong to package
			// java
			...
			if (importedClasses.contains(type)) {
				...
				p(name); // FQCN imported or not necessary, so generate short name
			} else {
				JClass outer = type.outer();

				if (outer != null) {
					String outerName = outer.name();
					JClass self = null;

					for (JClass j : importedClasses) {
						if (j instanceof JReferencedClass) {
					...
					if (self != null) {
						if (outer.isAssignableFrom(self)) {
							p(type.name().substring(outerName.length() + 1));
							break;
						} else {
							p(type.name());
							break;
	...
	/**
	 * Special character token we use to differenciate '>' as an operator and '>' as
	 * the end of the type arguments. The former uses '>' and it requires a
	 * preceding whitespace. The latter uses this, and it does not have a preceding
	 * whitespace.
	 */
	...
	final class ReferenceList {
		...
		public boolean collisions(JDefinedClass enclosingClass) {
			...
			for (JClass c : classes) {
				...
				JClass jclass = c.outer();
				if (jclass != null) {

					if (c instanceof JReferencedClass) {
						// JReferencedClass d = (JReferencedClass) c;
						// if (!d.isStatic()) {
						if (jclass.isAbstract()) {
							return true;
						}
					}
					return false; // avoid importing inner class to work around 6431987. Also see
					// jaxb issue 166
				}
			}
		...
		/**
		 * Return true iff this is strictly an id, meaning that there are no collisions
		 * with type names.
		 */
		public boolean isId() {

...
public class mxGraph extends mxEventSource {
	...
	/**
	 * Holds the version number of this release. Current version is 3.9.1.
	 */
	...
	/**
	 * Specifies if ports are enabled. This is used in <cellConnected> to update the
	 * respective style. Default is true.
	 */
	protected boolean portsEnabled = true;

	/**
	 * Value returned by getOverlap if isAllowOverlapParent returns true for the
	 * given cell. getOverlap is used in keepInside if isKeepInsideParentOnMove
	...
	/**
	 * Specifies the default parent to be used to insert new cells. This is used in
	 * getDefaultParent. Default is null.
	 */
	protected Object defaultParent;

	/**
	 * Specifies the alternate edge style to be used if the main control point on an
	 * edge is being doubleclicked. Default is null.
	 */
	 ...
	 * should be placed. Uses in getMaximumGraphBounds. Use a width or height of 0
	 * if you only want to give a upper, left corner.
	 */
	protected mxRectangle maximumGraphBounds = null;

	/**
	 * mxRectangle that specifies the minimum size of the graph canvas inside the
	 * scrollpane.
	 */
	protected mxRectangle minimumGraphSize = null;

	/**
	 * Border to be added to the bottom and right side when the container is being
	 * resized after the graph has been changed. Default is 0.
	 */
	protected int border = 0;

	/**
	 * Specifies if edges should appear in the foreground regardless of their order
	 * in the model. This has precendence over keepEdgeInBackground Default is
	 * false.
	 */
	...
	/**
	 * Specifies if edges that are cloned should be validated and only inserted if
	 * they are valid. Default is true.
	 */
	...
	/**
	 * Specifies if labels should be visible. This is used in getLabel. Default is
	 * true.
	 */
	...
	/**
	 * Specifies the maximum number of changes that should be processed to find the
	 * dirty region. If the number of changes is larger, then the complete grah is
	 * repainted. A value of zero will always compute the dirty region for any
	 * number of changes. Default is 1000.
	 */
	...
	protected mxIEventListener graphModelChangeHandler = new mxIEventListener() {
		public void invoke(Object sender, mxEventObject evt) {
			mxRectangle dirty = graphModelChanged((mxIGraphModel) sender,
					(List<mxUndoableChange>) ((mxUndoableEdit) evt.getProperty("edit")).getChanges());
			repaint(dirty);
		}
	};
	...
	/**
	 * Called when the graph model changes. Invokes processChange on each item of
	 * the given array to update the view accordingly.
	 */
	 ...
	 * 
	 * The first validation is required to compute the bounds of the graph while the
	 * second validation is required to apply the new translate.
	 */
	...
	/**
	 * Processes the changes and returns the minimal rectangle to be repainted in
	 * the buffer. A return value of null means no repaint is required.
	 */
	public mxRectangle processChanges(List<mxUndoableChange> changes, boolean invalidate, boolean ignoreDirty) {
	 ...
	 */
	public mxRectangle processChange(mxUndoableChange change, boolean invalidate, boolean ignoreDirty) {
	...
	/**
	 * Removes all cached information for the given cell and its descendants. This
	 * is called when a cell was removed from the model.
	 * 
	 * @param cell
	...
	public Map<String, Object> getCellStyle(Object cell) {
		Map<String, Object> style = (model.isEdge(cell)) ? stylesheet.getDefaultEdgeStyle()
	 ...
	 * short data URIs as defined in mxImageBundle to data URIs as defined in RFC
	 * 2397 of the IETF.
	 */
	 ...
	 *            String representing the new style of the cells.
	 * @param cells
	 *            Optional array of <mxCells> to set the style for. Default is the
	 *            selection cells.
	 */
	 ...
	 * If no cells are specified, then the selection cells are used. For example,
	 * this can be used to toggle mxConstants.STYLE_ROUNDED or any other style with
	 * a boolean value.
	 * 
	 * @param key
	 ...
	 *            Integer that represents the bit to be toggled.
	 * @param cells
	 *            Optional array of <mxCells> to change the style for. Default is
	 *            the selection cells.
	 */
	 ...
	 *            Boolean value to be used or null if the value should be toggled.
	 * @param cells
	 *            Optional array of cells to change the style for. If no cells are
	...
	/**
	 * Aligns the selection cells vertically or horizontally according to the given
	 * alignment.
	 * 
	 * @param align
	 ...
	 * cellsOrdered. This method fires mxEvent.ORDER_CELLS while the transaction is
	 * in progress.
	 * 
	 * @param back
	 *            Specifies if the cells should be moved to back.
	 * @param cells
	 *            Array of cells whose order should be changed. If null is specified
	 ...
	 * group. A group is only created if there is at least one entry in the given
	 * array of cells.
	 * 
	 * @param group
	 *            Cell that represents the target group. If null is specified then a
	 *            new group is created using createGroupCell.
	 * @param border
	 *            Integer that specifies the border between the child area and the
	 *            group bounds.
	 * @param cells
	 *            Optional array of cells to be grouped. If null is specified then
	 ...
	 * implementation computes the bounding box of the geometries of all vertices in
	 * the given children array. Edges are ignored. If the group cell is a swimlane
	 * the title region is added to the bounds.
	 */
	 ...
	 * group cell was given to the <group> function. The children are just for
	 * informational purpose, they will be added to the returned group later. Note
	 * that the returned group should have a geometry. The coordinates of which are
	 * later overridden.
	 * 
	 * @param cells
	 ...
	 * parent and removing the empty groups.
	 * 
	 * @param cells
	...
	/**
	 * Removes the selection cells from their parents and adds them to the default
	 * parent returned by getDefaultParent.
	 */
	...
	/**
	 * Updates the bounds of the given array of groups so that it includes all child
	 * vertices.
	 */
	...
	/**
	 * Clones all cells in the given array. To clone all children in a cell and add
	 * them to another graph:
	 * 
	 * <code>
	 ...
	 * default parent (typically default layer) of another graph g2, the following
	 * code is used:
	 * 
	 * <code>
	 ...
	 * terminal is removed. If a cloned edge is invalid and allowInvalidEdges is
	 * false, then a null pointer will be at this position in the returned array.
	 * Use getCloneableCells on the input array to only clone the cells where
	...
	/**
	 * Adds a new vertex into the given parent using value as the user object and
	 * the given coordinates as the geometry of the new vertex. The id and style are
	 * used for the respective properties of the new cell, which is returned.
	 * 
	 * @param parent
	...
	/**
	 * Adds a new edge into the given parent using value as the user object and the
	 * given source and target as the terminals of the new edge. The Id and style
	 * are used for the respective properties of the new cell, which is returned.
	 * 
	 * @param parent
	 ...
	 * terminals. This is a shortcut method.
	 * 
	 * @param edge
	 *            Edge to be inserted into the given parent.
	 * @param parent
	 *            Object that represents the new parent. If no parent is given then
	...
	/**
	 * Adds the cells to the parent at the given index, connecting each cell to the
	 * optional source and target terminal. The change is carried out using
	 * cellsAdded. This method fires mxEvent.ADD_CELLS while the transaction is in
	 * progress.
	 * 
	 * @param cells
	 ...
	 *            Optional index to insert the cells at. Default is to append.
	 * @param source
	 *            Optional source terminal for all inserted cells.
	 * @param target
	...
	public Object[] addCells(Object[] cells, Object parent, Integer index, Object source, Object target) {
		...
		try {
			...
			fireEvent(new mxEventObject(mxEvent.ADD_CELLS, "cells", cells, "parent", parent, "index", index, "source",
	...
	public void cellsAdded(Object[] cells, Object parent, Integer index, Object source, Object target, boolean absolute,
		...
		if (cells != null && parent != null && index != null) {
			...
			try {
				...
				fireEvent(new mxEventObject(mxEvent.CELLS_ADDED, "cells", cells, "parent", parent, "index", index,
						"source", source, "target", target, "absolute", absolute));
	 ...
	 * 
	 * @param cells
	 *            Array of cells to remove. If null is specified then the selection
	 *            cells which are deletable are used.
	 * @param includeEdges
	...
	/**
	 * Splits the given edge by adding a newEdge between the previous source and the
	 * given cell and reconnecting the source of the given edge to the given cell.
	 * Fires mxEvent.SPLIT_EDGE while the transaction is in progress.
	 * 
	 * @param edge
	 *            Object that represents the edge to be splitted.
	 * @param cells
	 *            Array that contains the cells to insert into the edge.
	 * @param newEdge
	...
	public Object splitEdge(Object edge, Object[] cells, Object newEdge, double dx, double dy) {
		...
		try {
			...
			fireEvent(new mxEventObject(mxEvent.SPLIT_EDGE, "edge", edge, "cells", cells, "newEdge", newEdge, "dx", dx,
					"dy", dy));
		} finally {
			model.endUpdate();
		}
	 ...
	 *            Boolean that specifies the visible state to be assigned.
	 * @param cells
	 *            Array of cells whose visible state should be changed. If null is
	 *            specified then the selection cells are used.
	 * @return Returns the cells whose visible state was changed.
	 */
	public Object[] toggleCells(boolean show, Object[] cells, boolean includeEdges) {
		...
		try {
			...
			fireEvent(new mxEventObject(mxEvent.TOGGLE_CELLS, "show", show, "cells", cells, "includeEdges",
					includeEdges));
		} finally {
			model.endUpdate();
		}
	...
	/**
	 * Sets the collapsed state of the selection cells without recursion. This is a
	 * shortcut method.
	 * 
	 * @param collapse
	 ...
	 *            Boolean that specifies the collapsed state to be assigned.
	 * @param recurse
	 *            Boolean that specifies if the collapsed state should be assigned
	 *            to all descendants.
	 * @return Returns the cells whose collapsed state was changed.
	 */
	 ...
	 * fires mxEvent.FOLD_CELLS while the transaction is in progress. Returns the
	 ...
	 * @param checkFoldable
	 *            Boolean indicating of isCellFoldable should be checked. Default is
	 *            false.
	 */
	 ...
	 * mxEvent.CELLS_FOLDED while the transaction is in progress. Returns the cells
	 * whose collapsed state was changed.
	 * 
	 * @param cells
	...
	public void cellsFolded(Object[] cells, boolean collapse, boolean recurse, boolean checkFoldable) {
		if (cells != null && cells.length > 0) {
			...
			try {
				...
				fireEvent(new mxEventObject(mxEvent.CELLS_FOLDED, "cells", cells, "collapse", collapse, "recurse",
						recurse));
			} finally {
				model.endUpdate();
			}
	...
	/**
	 * Swaps the alternate and the actual bounds in the geometry of the given cell
	 * invoking updateAlternateBounds before carrying out the swap.
	 * 
	 * @param cell
	...
	/**
	 * Updates or sets the alternate bounds in the given geometry for the given cell
	 * depending on whether the cell is going to be collapsed. If no alternate
	 * bounds are defined in the geometry and collapseToPreferredSize is true, then
	 * the preferred size is used for the alternate bounds. The top, left corner is
	 * always kept at the same location.
	 * 
	 * @param cell
	...
	/**
	 * Returns an array with the given cells and all edges that are connected to a
	 * cell or one of its descendants.
	 */
	 ...
	 * to get the new size. This function fires beforeUpdateSize and afterUpdateSize
	 * events.
	 * 
	 * @param cell
	...
	/**
	 * Updates the size of the given cell in the model using getPreferredSizeForCell
	 * to get the new size. This function fires mxEvent.UPDATE_CELL_SIZE.
	 * 
	 * @param cell
	 ...
	 * while the transaction is in progress. Returns the cells which have been
	 ...
	 * If extendParents is true, then the parent is extended if a child size is
	 * changed so that it overlaps with the parent.
	 * 
	 * @param cells
	...
	/**
	 * Resizes the parents recursively so that they contain the complete area of the
	 * resized child cell.
	 * 
	 * @param cell
	 ...
	 * given amount, adding them to the optional target cell. The location is the
	 * position of the mouse pointer as the mouse was released. The change is
	 * carried out using cellsMoved. This method fires mxEvent.MOVE_CELLS while the
	...
	public Object[] moveCells(Object[] cells, double dx, double dy, boolean clone, Object target, Point location) {
		if (cells != null && (dx != 0 || dy != 0 || clone || target != null)) {
			...
			try {
				...
				fireEvent(new mxEventObject(mxEvent.MOVE_CELLS, "cells", cells, "dx", dx, "dy", dy, "clone", clone,
	 ...
	 * disconnectGraph if disconnect is true. This method fires mxEvent.CELLS_MOVED
	...
	public void cellsMoved(Object[] cells, double dx, double dy, boolean disconnect, boolean constrain) {
		if (cells != null && (dx != 0 || dy != 0)) {
			...
			try {
				...
				fireEvent(new mxEventObject(mxEvent.CELLS_MOVED, "cells", cells, "dx", dx, "dy", dy, "disconnect",
						disconnect));
			} finally {
				model.endUpdate();
			}
	 ...
	 * its parent, according to the rules defined by getOverlap and
	 ...
	 * This result can then be passed to getConnectionPoint.
	 * 
	 * @param edge
	...
	/**
	 * Sets the connection constraint that describes the given connection point. If
	 * no constraint is given then nothing is changed. To remove an existing
	 ...
	 */
	public void setConnectionConstraint(Object edge, Object terminal, boolean source,
		...
		if (constraint != null) {
			...
			try {
				...
				if (constraint == null || constraint.point == null) {
					...
					setCellStyles((source) ? mxConstants.STYLE_EXIT_PERIMETER : mxConstants.STYLE_ENTRY_PERIMETER, null,
							cells);
				} else if (constraint.point != null) {
					setCellStyles((source) ? mxConstants.STYLE_EXIT_X : mxConstants.STYLE_ENTRY_X,
							String.valueOf(constraint.point.getX()), cells);
					setCellStyles((source) ? mxConstants.STYLE_EXIT_Y : mxConstants.STYLE_ENTRY_Y,
							String.valueOf(constraint.point.getY()), cells);

					// Only writes 0 since 1 is default
					if (!constraint.perimeter) {
						setCellStyles((source) ? mxConstants.STYLE_EXIT_PERIMETER : mxConstants.STYLE_ENTRY_PERIMETER,
								"0", cells);
					} else {
						setCellStyles((source) ? mxConstants.STYLE_EXIT_PERIMETER : mxConstants.STYLE_ENTRY_PERIMETER,
								null, cells);
	 ...
	 * @param terminal
	 *            New terminal to be used.
	 * @param source
	 *            Specifies if the new terminal is the source or target.
	 * @param constraint
	 *            Optional constraint to be used for this connection.
	...
	public void cellConnected(Object edge, Object terminal, boolean source, mxConnectionConstraint constraint) {
		if (edge != null) {
			...
			try {
				...
				fireEvent(new mxEventObject(mxEvent.CELL_CONNECTED, "edge", edge, "terminal", terminal, "source",
	 ...
	 * the port should be referenced by the ID in either the
	 * mxConstants.STYLE_SOURCE_PORT or the or the mxConstants.STYLE_TARGET_PORT.
	 * Note that a port should not be movable. This implementation always returns
	 * false.
	 * 
	...
	/**
	 * Returns the terminal to be used for a given port. This implementation always
	...
	/**
	 * Returns the offset to be used for the cells inside the given cell. The root
	 * and layer cells may be identified using mxGraphModel.isRoot and
	 ...
	 * is specified then the selection cell is used. The cell is only used if
	 * <isValidRoot> returns true.
	 * 
	 * @param cell
	 */
	...
	/**
	 * Uses the root of the model as the root of the displayed cell hierarchy and
	 * selects the previous root.
	 */
	public void home() {
		Object current = getCurrentRoot();
	...
	/**
	 * Returns the bounding box of the given cell including all connected edges if
	 * includeEdge is true.
	 */
	 ...
	 */
	public mxRectangle getBoundsForCells(Object[] cells, boolean includeEdges, boolean includeDescendants,
			boolean boundingBox) {
		mxRectangle result = null;
	...
	/**
	 * Fires a repaint event. The optional region is the rectangle that needs to be
	 * repainted.
	 */
	 ...
	 * implementation uses <mxGraphModel.isCollapsed>. Subclassers can override this
	 * to implement specific collapsed states for cells in only one graph, that is,
	 * without affecting the collapsed state of the cell.
	 * 
	 * When using dynamic filter expressions for the collapsed state, then the graph
	 * should be revalidated after the filter expression has changed.
	 * 
	 * @param cell
	...
	public boolean isOrthogonal(mxCellState edge) {
		...
		return tmp == mxEdgeStyle.SegmentConnector || tmp == mxEdgeStyle.ElbowConnector || tmp == mxEdgeStyle.SideToSide
				|| tmp == mxEdgeStyle.TopToBottom || tmp == mxEdgeStyle.EntityRelation
	 ...
	 * valid, a return value of '' means it's not valid, but do not display an error
	 * message. Any other (non-empty) string returned from this method is displayed
	 * as an error message when trying to connect an edge to a source and target.
	 * This implementation uses the multiplicities, as well as multigraph and
	 * allowDanglingEdges to generate validation errors.
	 * 
	 * @param edge
	 *            Cell that represents the edge to validate.
	 * @param source
	...
	/**
	 * Hook method for subclassers to return an error message for the given edge and
	 * terminals. This implementation returns null.
	 * 
	 * @param edge
	 *            Cell that represents the edge to validate.
	 * @param source
	...
	/**
	 * Checks all multiplicities that cannot be enforced while the graph is being
	 * modified, namely, all multiplicities that require a minimum of 1 edge.
	 * 
	 * @param cell
	...
	/**
	 * Hook method for subclassers to return an error message for the given cell and
	 * validation context. This implementation returns null.
	 * 
	 * @param cell
	 ...
	 * the part that contains the title, depending on the horizontal style. The
	 * return value is an <mxRectangle> with either width or height set as
	 * appropriate.
	 * 
	 * @param swimlane
	...
	/**
	 * Returns the image URL for the given cell state. This implementation returns
	 * the value stored under <mxConstants.STYLE_IMAGE> in the cell style.
	 * 
	 * @param state
	 ...
	 * relative geometry if cellsLocked is false.
	 * 
	 * @param cell
	...
	public boolean isCellEditable(Object cell) {
		...
		return isCellsEditable() && !isCellLocked(cell) && mxUtils.isTrue(style, mxConstants.STYLE_EDITABLE, true);
	}
	 ...
	 *
	 * Returns true if the given terminal point is movable. This is independent from
	 * isCellConnectable and isCellDisconnectable and controls if terminal points
	 * can be moved in the graph if the edge is not connected. Note that it is
	 * required for this to return true to connect unconnected edges. This
	...
	/**
	 * Returns true if the given cell is movable. This implementation always returns
	 * true.
	 * 
	 * @param cell
	 *            Cell whose movable state should be returned.
	...
	public boolean isCellDeletable(Object cell) {
		...
		return isCellsDeletable() && mxUtils.isTrue(style, mxConstants.STYLE_DELETABLE, true);
	}
	...
	/**
	 * Returns the constant true. This does not use the cloneable field to return a
	 * value for a given cell, it is simply a hook for subclassers to disallow
	 * cloning of individual cells.
	 */
	public boolean isCellCloneable(Object cell) {
		mxCellState state = view.getState(cell);
		Map<String, Object> style = (state != null) ? state.getStyle() : getCellStyle(cell);

		return isCellsCloneable() && mxUtils.isTrue(style, mxConstants.STYLE_CLONEABLE, true);
	}
	 ...
	 * terminal. This returns <disconnectable> for all given cells if <isLocked>
	...
	public boolean isLabelMovable(Object cell) {
		return !isCellLocked(cell)
				&& ((model.isEdge(cell) && isEdgeLabelsMovable()) || (model.isVertex(cell) && isVertexLabelsMovable()));
	}
	 ...
	 * after a change of the label. This implementation returns autoSize for all
	 * given cells or checks if the cell style does specify
	...
	public boolean isAutoSizeCell(Object cell) {
		...
		return isAutoSizeCells() || mxUtils.isTrue(style, mxConstants.STYLE_AUTOSIZE, false);
	}
	 ...
	 * This implementation sets autoSize to the given parameter.
	 * 
	 * @param value
	...
	/**
	 * Returns true if the parent of the given cell should be extended if the child
	 * has been resized so that it overlaps the parent. This implementation returns
	 * ExtendParents if cell is not an edge.
	 * 
	 * @param cell
	 ...
	 * according to the rules defined by getOverlap and isAllowOverlapParent. This
	 * implementation returns false for all children of edges and
	 * isConstrainChildren() otherwise.
	 */
	...
	/**
	 * Returns true if the given cell is a valid source for new connections. This
	 * implementation returns true for all non-null values and is called by is
	 * called by <isValidConnection>.
	 * 
	 * @param cell
	...
	/**
	 * Returns true if the given target cell is a valid target for source. This is a
	 * boolean implementation for not allowing connections between certain pairs of
	 * vertices and is called by <getEdgeValidationError>. This implementation
	 * returns true if <isValidSource> returns true for the source and
	 * <isValidTarget> returns true for the target.
	 * 
	 * @param source
	 ...
	 * If <isAllowOverlapParent> returns false for the given cell, then this method
	 * returns 0.
	 * 
	 * @param cell
	...
	/**
	 * Returns true if the given cell is allowed to be placed outside of the parents
	 * area.
	 * 
	 * @param cell
	 ...
	 * cells. This returns true if the cell is a swimlane, has children and is not
	 * collapsed, or if splitEnabled is true and isSplitTarget returns true for the
	 * given arguments
	 * 
	 * @param cell
	...
	public boolean isValidDropTarget(Object cell, Object[] cells) {
		return cell != null && ((isSplitEnabled() && isSplitTarget(cell, cells)) || (!model.isEdge(cell)
				&& (isSwimlane(cell) || (model.getChildCount(cell) > 0 && !isCellCollapsed(cell)))));
	}

	/**
	 * Returns true if split is enabled and the given edge may be splitted into two
	 * edges with the given cell as a new terminal between the two.
	 * 
	 * @param target
	 *            Object that represents the edge to be splitted.
	 * @param cells
	...
	public boolean isSplitTarget(Object target, Object[] cells) {
		if (target != null && cells != null && cells.length == 1) {
			...
			return (model.isEdge(target) && isCellConnectable(cells[0])
					&& getEdgeValidationError(target, model.getTerminal(target, true), cells[0]) == null
	 ...
	 * nearest ancestor that may be used as a drop target for the given cells. If
	 * the given array contains a swimlane and swimlaneNesting is false then this
	 * always returns null. If no cell is given, then the bottommost swimlane at the
	 * location of the given event is returned.
	 * 
	...
	public Object getDropTarget(Object[] cells, Point pt, Object cell) {
		...
		/*
		 * else if (swimlane != null) { // Checks if the cell is an ancestor of the
		 * swimlane // under the mouse and uses the swimlane in that case Object tmp =
		 * model.getParent(swimlane);
		 * 
		...
		return (model.getParent(cell) != model.getRoot() && !mxUtils.contains(cells, cell)) ? cell : null;
	};
	...
	/**
	 * Sets the default parent to be returned by getDefaultParent. Set this to null
	 * to return the first child of the root in getDefaultParent.
	 */
	...
	/**
	 * Returns all visible edges connected to the given cell without loops. If the
	 * optional parent argument is specified, then only child edges of the given
	 * parent are returned.
	 * 
	 * @param cell
	 *            Cell whose connections should be returned.
	...
	/**
	 * Returns the visible incoming edges for the given cell. If the optional parent
	 * argument is specified, then only child edges of the given parent are
	 * returned.
	 * 
	 * @param cell
	 *            Cell whose incoming edges should be returned.
	 ...
	 * optional parent argument is specified, then only edges are returned where the
	 * opposite is in the given parent cell.
	 * 
	 * @param cell
	 *            Cell whose edges should be returned.
	 * @param parent
	 *            Optional parent. If specified the opposite end of any edge must be
	 *            a direct child of that parent in order for the edge to be
	 *            returned.
	 * @param incoming
	 ...
	 * @param parent
	 *            Optional parent. If specified the opposite end of any edge must be
	 *            a child of that parent in order for the edge to be returned. The
	 *            recurse parameter specifies whether or not it must be the direct
	 *            child or the parent just be an ancestral parent.
	 * @param incoming
	 *            Specifies if incoming edges should be included in the result.
	 * @param outgoing
	 *            Specifies if outgoing edges should be included in the result.
	...
	public Object[] getEdges(Object cell, Object parent, boolean incoming, boolean outgoing, boolean includeLoops,
		...
		while (it.hasNext()) {
			...
			if ((includeLoops && source == target) || ((source != target) && ((incoming && target == cell
					&& (parent == null || isValidAncestor(source, parent, recurse)))
	...
	/**
	 * Returns all distincts visible opposite cells for the specified terminal on
	 * the given edges.
	 * 
	 * @param edges
	...
	public Object[] getEdgesBetween(Object source, Object target, boolean directed) {
		...
		for (int i = 0; i < edges.length; i++) {
			mxCellState state = view.getState(edges[i]);
			Object src = (state != null) ? state.getVisibleTerminal(true) : view.getVisibleTerminal(edges[i], true);
			Object trg = (state != null) ? state.getVisibleTerminal(false) : view.getVisibleTerminal(edges[i], false);
	 ...
	 * from the given point (x0, y0) rightwards and downwards depending on
	 * rightHalfpane and bottomHalfpane.
	 * 
	 * @param x0
	 ...
	 *            Boolean indicating if the cells in the bottom halfpane from the
	 *            origin should be returned.
	...
	/**
	 * Returns all visible children in the given parent which do not have incoming
	 * edges. If the result is empty then the with the greatest difference between
	 * incoming and outgoing edges is returned. This takes into account edges that
	 * are being promoted to the given root due to invisible children or collapsed
	 * cells.
	 * 
	 * @param parent
	...
	/**
	 * Returns all visible children in the given parent which do not have incoming
	 * edges. If the result is empty then the children with the maximum difference
	 * between incoming and outgoing edges are returned. This takes into account
	 * edges that are being promoted to the given root due to invisible children or
	 * collapsed cells.
	 * 
	 * @param parent
	 *            Cell whose children should be checked.
	 * @param isolate
	 *            Specifies if edges should be ignored if the opposite end is not a
	 *            child of the given parent cell.
	 * @return List of tree roots in parent.
	 */
	 ...
	 *            child of the given parent cell.
	 * @param invert
	 *            Specifies if outgoing or incoming edges should be counted for a
	 *            tree root. If false then outgoing edges will be counted.
	 * @return List of tree roots in parent.
	 */
	 ...
	 * @param directed
	 *            Optional boolean indicating if edges should only be traversed from
	 *            source to target. Default is true.
	 * @param visitor
	 *            Visitor that takes the current vertex and the incoming edge. The
	 *            traversal stops if the function returns false.
	 * @param edge
	 *            Optional <mxCell> that represents the incoming edge. This is null
	...
	/**
	 * Selects the next, parent, first child or previous cell, if all arguments are
	 * false.
	 * 
	 * @param isNext
	 * @param isParent
	 ...
	 * recursively, starting at the default parent. Use <code>selectAll</code> to
	 * select all cells.
	 * 
	 * @param vertices
	 ...
	 * is specified. Use <code>selectAll</code> to select all cells.
	 * 
	 * @param vertices
	 *            Boolean indicating if vertices should be selected.
	 * @param edges
	...
	public void selectCells(final boolean vertices, final boolean edges, Object parent) {
		...
		Collection<Object> cells = mxGraphModel.filterDescendants(getModel(), new mxGraphModel.Filter() {
			 ...
			 */
			public boolean filter(Object cell) {
				return view.getState(cell) != null && model.getChildCount(cell) == 0
	 ...
	 * parent if no parent is specified. To select leaf vertices and/or edges use
	 * <selectCells>.
	 * 
	 * @param parent
	 *            Optional <mxCell> whose children should be selected. Default is
	 *            <defaultParent>.
	 */
	 ...
	 * descendants are painted here. This method invokes cellDrawn after the cell,
	 * but not its descendants have been painted.
	 * 
	 * @param canvas
	 *            Canvas onto which the cell should be drawn.
	 * @param state
	 *            State of the cell to be drawn.
	 * @param drawLabel
	...
	/**
	 * Called when a cell has been painted as the specified object, typically a DOM
	 * node that represents the given cell graphically in a document.
	 */

...
public class mxMultiplicity {
	 ...
	 * passed to mxUtils.isNode together with the source or target vertex value as
	 * the first argument.
	 */
	protected String type;

	/**
	 * Optional string that specifies the attributename to be passed to mxCell.is to
	 * check if the rule applies to a cell.
	 */
	...
	/**
	 * Defines the maximum number of connections for which this rule applies. A
	 * value of 'n' means unlimited times. Default is 'n'.
	 */
	protected String max = "n";

	/**
	 * Holds an array of strings that specify the type of neighbor for which this
	 * rule applies. The strings are used in mxCell.is on the opposite terminal to
	 * check if the rule applies to the connection.
	 */
	 ...
	 * connections for which the rule applies is smaller than min or greater than
	 * max.
	 */
	protected String countError;

	/**
	 * Holds the localized error message to be displayed if the type of the neighbor
	 * for a connection does not match the rule.
	 */
	 ...
	 * 
	 */
	public mxMultiplicity(boolean source, String type, String attr, String value, int min, String max,
			Collection<String> validNeighbors, String countError, String typeError, boolean validNeighborsAllowed) {
		this.source = source;
		this.type = type;
		this.attr = attr;
		this.value = value;
	 ...
	 * Function: check
	 * 
	 * Checks the multiplicity for the given arguments and returns the error for the
	 * given connection or null if the multiplicity does not apply.
	 * 
	 * Parameters:
	 * 
	 * graph - Reference to the enclosing graph instance. edge - Cell that
	 * represents the edge to validate. source - Cell that represents the source
	 * terminal. target - Cell that represents the target terminal. sourceOut -
	 * Number of outgoing edges from the source terminal. targetIn - Number of
	 * incoming edges for the target terminal.
	 */

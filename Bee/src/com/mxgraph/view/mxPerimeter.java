...
/**
 * Provides various perimeter functions to be used in a style as the value of
 * mxConstants.STYLE_PERIMETER. Alternately, the mxConstants. PERIMETER_*
 * constants can be used to reference a perimeter via the mxStyleRegistry.
 */
public class mxPerimeter {
	...
	public interface mxPerimeterFunction {
		 ...
		 * @param next
		 *            Point that represents the nearest neighbour point on the given
		 *            edge.
		 * @param orthogonal
	...
	/**
	 * Describes a triangle perimeter. See RectanglePerimeter for a description of
	 * the parameters.
	 */
	public static mxPerimeterFunction TrianglePerimeter = new mxPerimeterFunction() {
		 ...
		 * 
		 * @see
		 * com.mxgraph.view.mxPerimeter.mxPerimeterFunction#apply(com.mxgraph.utils.
		 * mxRectangle, com.mxgraph.view.mxCellState, com.mxgraph.view.mxCellState,
		 * boolean, com.mxgraph.utils.mxPoint)
		 */
		public mxPoint apply(mxRectangle bounds, mxCellState vertex, mxPoint next, boolean orthogonal) {
			Object direction = (vertex != null)
					? mxUtils.getString(vertex.style, mxConstants.STYLE_DIRECTION, mxConstants.DIRECTION_EAST)
			...
			if (base) {
				if (orthogonal && ((vertical && next.getX() >= start.getX() && next.getX() <= end.getX())
						|| (!vertical && next.getY() >= start.getY() && next.getY() <= end.getY()))) {
					if (vertical) {
				...
				if (orthogonal) {
					...
					if (next.getY() >= y && next.getY() <= y + h) {
						pt.setX((vertical) ? cx : ((direction.equals(mxConstants.DIRECTION_WEST)) ? x + w : x));
						pt.setY(next.getY());
					} else if (next.getX() >= x && next.getX() <= x + w) {
						pt.setX(next.getX());
						pt.setY((!vertical) ? cy : ((direction.equals(mxConstants.DIRECTION_NORTH)) ? y + h : y));
					}
				...
				if ((vertical && next.getX() <= x + w / 2) || (!vertical && next.getY() <= y + h / 2)) {
					result = mxUtils.intersection(next.getX(), next.getY(), cx, cy, start.getX(), start.getY(),
							corner.getX(), corner.getY());
				} else {
					result = mxUtils.intersection(next.getX(), next.getY(), cx, cy, corner.getX(), corner.getY(),
							end.getX(), end.getY());
				}
			}
	...
	/**
	 * Describes a hexagon perimeter. See RectanglePerimeter for a description of
	 * the parameters.
	 */
	public static mxPerimeterFunction HexagonPerimeter = new mxPerimeterFunction() {
		public mxPoint apply(mxRectangle bounds, mxCellState vertex, mxPoint next, boolean orthogonal) {
			...
			if ((px < x) && (py < y) || (px < x) && (py > y + h) || (px > x + w) && (py < y)
					|| (px > x + w) && (py > y + h)) {
				orthogonal = false;
			}

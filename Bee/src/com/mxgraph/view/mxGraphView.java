...
public class mxGraphView extends mxEventSource {
	...
	public void scaleAndTranslate(double scale, double dx, double dy) {
		...
		fireEvent(new mxEventObject(mxEvent.SCALE_AND_TRANSLATE, "scale", scale, "previousScale", previousScale,
	...
	/**
	 * Sets the current scale and revalidates the view. Fires a "scale" event after
	 * calling revalidate. Revalidate is only called if isEventsEnabled.
	 * 
	 * @param value
	...
	/**
	 * Removes the state of the given cell and all descendants if the given cell is
	...
	/**
	 * First validates all bounds and then validates all points recursively on all
	 * visible cells.
	 */
	public void validate() {
		mxRectangle graphBounds = getBoundingBox(
	...
	/**
	 * Returns the bounding box of the shape and the label for the given cell state
				...
				if (!model.isEdge(state.getCell())) {
					...
					if (geo.isRelative() && pState != null) {
						if (model.isEdge(pState.getCell())) {
							...
							if (orig != null) {
								origin.setX(origin.getX() + (orig.getX() / scale) - pState.getOrigin().getX()
										- translate.getX());
								origin.setY(origin.getY() + (orig.getY() / scale) - pState.getOrigin().getY()
										- translate.getY());
							}
						} else {
	...
	/**
	 * Updates the absoluteOffset of the given vertex cell state. This takes into
	 * account the label position styles.
	 * 
	 * @param state
	...
	public void updateLabel(mxCellState state) {
		...
		// state
		if (label != null && label.length() > 0 && !graph.isHtmlLabel(state.getCell())
			...
			String[] lines = mxUtils.wordWrap(label, mxUtils.getFontMetrics(mxUtils.getFont(state.getStyle())),
	...
	public double getWordWrapWidth(mxCellState state) {
		...
		if (horizontal) {
			w = (state.getWidth() / scale) - 2 * mxConstants.LABEL_INSET
			...
			w = (state.getHeight() / scale) - 2 * mxConstants.LABEL_INSET
					- 2 * mxUtils.getDouble(style, mxConstants.STYLE_SPACING)
	...
	public void updateLabelBounds(mxCellState state) {
		...
		if (overflow.equals("fill")) {
			...
			state.setLabelBounds(mxUtils.getLabelPaintBounds(state.getLabel(), style, graph.isHtmlLabel(cell),
					state.getAbsoluteOffset(), vertexBounds, scale, graph.getModel().isEdge(cell)));
	 ...
	 *            Cell state whose initial terminal points should be updated.
	 */
	public void updateFixedTerminalPoint(mxCellState edge, mxCellState terminal, boolean source,
		...
		if (pt == null && terminal == null) {
			...
			if (pt != null) {
				pt = new mxPoint(scale * (translate.getX() + pt.getX() + orig.getX()),
	...
	public mxPoint transformControlPoint(mxCellState state, mxPoint pt) {
		...
		return new mxPoint(scale * (pt.getX() + translate.getX() + origin.getX()),
				scale * (pt.getY() + translate.getY() + origin.getY()));
	}

	/**
	 * Returns the edge style function to be used to compute the absolute points for
	 * the given state, control points and terminals.
	 */
	...
	/**
	 * Updates the absolute terminal point in the given state for the given start
	 * and end state, where start is the source if source is true.
	 * 
	 * @param edge
	...
	public void updateFloatingTerminalPoint(mxCellState edge, mxCellState start, mxCellState end, boolean source) {
		...
		border += mxUtils.getDouble(edge.getStyle(),
				(source) ? mxConstants.STYLE_SOURCE_PERIMETER_SPACING : mxConstants.STYLE_TARGET_PERIMETER_SPACING);
		mxPoint pt = getPerimeterPoint(start, next, graph.isOrthogonal(edge), border);
		edge.setAbsoluteTerminalPoint(pt, source);
	}

	/**
	 * Returns a cell state that represents the source or target terminal or port
	 * for the given edge.
	 */
	 ...
	 * the perimeter and the line between the center of the shape and the given
	 * point.
	 */
	 ...
	 *            intersection of the perimeter and the line between the next and
	...
	public double getRoutingCenterX(mxCellState state) {
		float f = (state.getStyle() != null) ? mxUtils.getFloat(state.getStyle(), mxConstants.STYLE_ROUTING_CENTER_X)
				: 0;
	...
	public double getRoutingCenterY(mxCellState state) {
		float f = (state.getStyle() != null) ? mxUtils.getFloat(state.getStyle(), mxConstants.STYLE_ROUTING_CENTER_Y)
				: 0;
	...
	/**
	 * Returns the nearest point in the list of absolute points or the center of the
	 * opposite terminal.
	 * 
	 * @param edge
	...
	/**
	 * Returns the nearest ancestor terminal that is visible. The edge appears to be
	 * connected to this terminal on the display.
	 * 
	 * @param edge
	 *            Cell whose visible terminal should be returned.
	 * @param source
	 *            Boolean that specifies if the source or target terminal should be
	 *            returned.
	...
	/**
	 * Updates the given state using the bounding box of the absolute points. Also
	 * updates terminal distance, length and segments.
	 * 
	 * @param state
	...
	/**
	 * Returns the absolute point on the edge for the given relative geometry as a
	 * point. The edge is represented by the given cell state.
	 * 
	 * @param state
	...
	/**
	 * Gets the relative point that describes the given, absolute label position for
	 * the given edge state.
	 */
	 ...
	 * states that are not null, that is, the returned array may have less elements
	...
	/**
	 * Returns the state for the given cell or null if no state is defined for the
	 * cell.
	 * 
	 * @param cell
	...
	/**
	 * Returns the cell state for the given cell. If create is true, then the state
	 * is created if it does not yet exist.
	 * 
	 * @param cell
	 *            Cell for which a new state should be returned.
	 * @param create
	 *            Boolean indicating if a new state should be created if it does not
	 *            yet exist.
	 * @return Returns the state for the given cell.
	 */

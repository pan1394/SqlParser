 ...
 * swimlane to its siblings, if siblings is true, and its ancestors, if bubbling
 * is true.
 */
public class mxSwimlaneManager extends mxEventSource {
	...
	/**
	 * Called for each swimlane which has been added. This finds a reference sibling
	 * swimlane and applies its size to the newly added swimlane. If no sibling can
	 * be found then the parent swimlane is resized so that the new swimlane fits
	 * into the parent swimlane.
	 */
					...
					if (!this.isSwimlaneIgnored(cells[i])) {
						...
						if (geo != null) {
							...
							while (current != null) {
								...
								mxRectangle tmp = (graph.isSwimlane(current)) ? graph.getStartSize(current)
										: new mxRectangle();

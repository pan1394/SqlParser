...
public class mxGraphStructure {
	...
	/**
	 * A helper function for getting a leaf vertex (degree <= 1), not taking into
	 * account edge direction - for internal use
	 * 
	 * @param aGraph
	 * @return the first undirected leaf that could be found in the graph, null if
	 *         none
	 */
	...
	/**
	 * @param aGraph
	 * @return true if the graph has the structure of a tree, regardless of edge
	 *         direction
	 */
	 ...
	 * @param aGraph
	 * @param omitVertex
	 *            vertices in this array will be omitted, set this parameter to null
	 *            if you don't want this feature
	 * @return a vertex that has lowest degree, or one of those in case if there are
	 *         more
	 */
	 ...
	 * @param sourceVertex
	 * @param targetVertex
	 * @return Returns true if the two vertices are connected directly by an edge.
	 *         If directed, the result is true if they are connected by an edge that
	 *         points from source to target, if false direction isn't takein into
	 *         account, just connectivity.
	 */
	...
	public static void makeConnected(mxAnalysisGraph aGraph) {
		// an early check, to avoid running getGraphComponents() needlessly, which is
		// CPU intensive
	...
	public static Object[][] getGraphComponents(mxAnalysisGraph aGraph) {
		...
		while (unvisitedVertexList.size() > 0) {
			...
			// if not, create a new component and run a BFS populating the component and
			// reducing the unvisited list
			if (!isInComponent) {
		...
		if (isTree(aGraph)) {
			...
			for (int i = 0; i < bFSList.size(); i++) {
				...
				for (int j = 0; j < neighbors.length; j++) {
					...
					if (childIndex > i) {
						// parentVertex is parent of currVertex, so the edge must be directed from
						// parentVertex to currVertex
	 ...
	 * @param vertexOne
	 * @param vertexTwo
	 * @return an edge that directly connects <b>vertexOne</b> and <b>vertexTwo</b>
	 *         regardless of direction, null if they are not connected directly
	 */
	...
	/**
	 * @param graph
	 * @return Returns true if the graph has at least one cycle, taking edge
	 ...
	 * @param parent
	 * @param properties
	 * @return A helper function for <b>isDirectedCyclic</b> and it isn't for
	 *         general use. It returns a node that hasn't incoming or outgoing
	 *         edges. It could be considered a "leaf" in a directed graph, but this
	 *         definition isn't formal.
	 */
	 ...
	 * @param value
	 *            - desired value
	 * @return the first vertex with the wanted value. If none are found, null is
	 *         returned
	 */
	...
	public static boolean isCutVertex(mxAnalysisGraph aGraph, Object vertex) {
		...
		if (aGraph.getEdges(vertex, null, true, true, false, true).length >= 2) {
			...
			Object newVertex = getVertexWithValue(aGraphCopy, (int) aGraph.getGenerator().getCostFunction()
					.getCost(new mxCellState(graph.getView(), vertex, null)));
	...
	public static boolean isCutEdge(mxAnalysisGraph aGraph, Object edge) {
		...
		if (aGraph.getTerminal(edge, false) != null || aGraph.getTerminal(edge, true) != null) {
			...
			int currSrcValue = (int) costFunctionCopy
					.getCost(new mxCellState(viewCopy, aGraphCopy.getTerminal(currEdge, true), null));
			int currDestValue = (int) costFunctionCopy
					.getCost(new mxCellState(viewCopy, aGraphCopy.getTerminal(currEdge, false), null));
			int i = 0;

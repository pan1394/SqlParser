...
public class mxGraphGenerator {
	...
	// private mxGeneratorFunction generatorFunction = new
	// mxGeneratorRandomFunction(0,1,2);
	// private mxGeneratorFunction generatorFunction = new
	// mxGeneratorConstFunction(1.5);
	// private mxGeneratorFunction generatorFunction = new
	// mxGeneratorRandomIntFunction(0, 20);
		...
		for (int j = 0; j < numRows; j++) {
			for (int i = 0; i < numColumns; i++) {
				...
				if (j > 0) {
					...
					graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[vertexCount - numColumns],
							currVertex);
				}
	...
	/**
	 * Sets the physical spacing between vertices in a grid graph. This works for
	 * now only for a graph generated with mxGraphCreator.getGridGraph() only after
	 * creating the graph
	 * 
	 * @param aGraph
	 * @param xSpacing
	 ...
	 *            - number of columns in the grid graph
	 */
	public void setGridGraphSpacing(mxAnalysisGraph aGraph, double xSpacing, double ySpacing, int numColumns,
			int numRows) {
		mxGraph graph = aGraph.getGraph();
	...
	/**
	 * @param aGraph
	 * @param numVerticesGroup1
	 *            number of vertices in group 1
	 * @param numVerticesGroup2
	...
	public void getBipartiteGraph(mxAnalysisGraph aGraph, int numVerticesGroup1, int numVerticesGroup2) {
		...
		for (int j = 0; j < numVerticesGroup2; j++) {
			...
			int edgeNum = aGraph.getOpposites(aGraph.getEdges(currVertex, null, true, true, false, true), currVertex,
					true, true).length;
	...
	/**
	 * Sets the physical spacing between vertices in a bipartite graph. This works
	 * for now only for a graph generated with mxGraphCreator.getBipartiteGraph()
	 ...
	 *            - number of vertices in group 2
	 * @param vertexSpacing
	 *            - vertical spacing between vertices in the same group
	 * @param groupSpacing
	 *            - spacing between groups
	 */
	public void setBipartiteGraphSpacing(mxAnalysisGraph aGraph, int numVerticesGroup1, int numVerticesGroup2,
			double vertexSpacing, double groupSpacing) {
		if (numVerticesGroup1 < 0 || numVerticesGroup2 < 0) {
			throw new IllegalArgumentException();
		}
	 ...
	 * @param xDim
	 * @param yDim
	 * @return a knight graph of size <b>xDim</b> x <b>yDim</b> Note that the
	 *         minimum size is 3x3
	 */
	public void getKnightGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
		...
		for (int i = 0; i < (xDim * yDim); i++) {
			...
			for (int j = 0; j < neighborMoves.length; j++) {
				// connect current with the possible move that has minimum number of its
				// (possible moves)
	 ...
	 * @param aGraph
	 * @param xDim
	 *            x dimension of chess-board, size starts from 1
	 * @param yDim
	 *            y dimension of chess-board, size starts from 1
	 * @param xCoord
	 *            x coordinate on the chess-board, coordinate starts from 1
	 * @param yCoord
	 *            y coordinate on the chess-board, coordinate starts from 1
	 * @return a list of ALL vertexes which would be valid moves from the current
	 *         position, regardless if they were visited or not Note that both
	 *         dimensions and both coordinates must be positive
	 */
	...
	/**
	 * use this only with the grid graph, and various chess-board graphs, because of
	 * vertex ordering
	 * 
	 * @param xDim
	 *            x dimension of chess-board, size starts from 1
	 ...
	 * @param yCoord
	 *            y coordinate on the chess-board, coordinate starts from 1
	 * @return vertex on the desired coordinates. Note that both dimensions and both
	 ...
	 * @param yDim
	 * @param weights
	 *            Return a king graph of size <b>xDim</b> x <b>yDim</b> Note that
	 *            the minimum size is 4x4
	 */
	 ...
	 * @param aGraph
	 * @param xDim
	 *            x dimension of the chessboard
	 * @param yDim
	 *            y dimension of the chessboard
	 * @param xCoord
	 *            the current x position of the king
	 * @param yCoord
	 *            the current y position of the king
	 * @return list of all possible moves of a king from the specified position Note
	 *         that both dimensions and both coordinates must be positive
	 */
	...
	/**
	 * Sets the physical spacing between vertices in a path graph. This works for
	 * now only for a graph generated with mxGraphCreator.getPathGraph() only after
	 * creating the graph
	 * 
	 * @param aGraph
	 * @param spacing
	 ...
	 * @param branchSize
	 *            number of vertices in a single branch (minimum >= 2) Returns a
	 *            friendship windmill graph (aka Dutch windmill)
	 */
	public void getFriendshipWindmillGraph(mxAnalysisGraph aGraph, int numBranches, int branchSize) {
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < branchSize; j++) {
				vertices[vertexCount] = graph.insertVertex(parent, null, new Integer(vertexCount).toString(), 0, 0, 25,
						25);
				vertexCount++;
			}
		}

		vertices[numVertices - 1] = graph.insertVertex(parent, null, new Integer(numVertices - 1).toString(), 0, 0, 25,
				25);
	...
	// NOTE the inside code is delicate, so please change it only if you know what
	// you're doing
	/**
	 * Sets the layout of a windmill graph. Use this method only for graphs
	 * generated with mxGraphGenerator.getWindmillGraph() and
	 * getFriendshitWindmillGraph()
	 * 
	 * @param aGraph
	 * @param numBranches
	 ...
	 * @param graphSize
	 */
	public void setWindmillGraphLayout(mxAnalysisGraph aGraph, int numBranches, int numVerticesInBranch,
			double graphSize) {
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < numVerticesInBranch; j++) {
				...
				if (isBranchSizeEven && j == middleIndex - 1) {
					// full size
					currRatio = ((double) i - (0.0005f * graphSize / Math.pow(numVerticesInBranch, 1)))
							/ (double) numVertexesInPerimeter;
					...
					if (j + 1 < middleIndex) {
						// before middle
						currRatio = ((double) (i - 1f / Math.pow(currSize, 0.25)
								+ 0.00000000000015f * Math.pow(currSize, 4)) / (double) numVertexesInPerimeter);
					} else {
						// after middle
						currRatio = ((double) (i + 1f / Math.pow(currSize, 0.25)
								- 0.00000000000015f * Math.pow(currSize, 4)) / (double) numVertexesInPerimeter);
	...
	/**
	 * A helper function that calculates the ring size for a windmill graph, based
	 * on the index of a vertex in a brach and branch size. - for internal use
	 * 
	 * @param currVertex
	 *            - starting from 1
	 * @param branchSize
	 ...
	 *            number of vertexes
	 * @param numEdges
	 *            number of edges (may be inaccurate if <b>forceConnected</b> is set
	 *            to true
	 * @param allowSelfLoops
	 *            if true, there will be a chance that self loops will be generated
	 *            too
	 * @param allowMultipleEdges
	 *            if true, there will be a chance that multiple edges will be
	 *            generated (multiple edges between the same two vertices)
	 * @param forceConnected
	 *            if true the resulting graph will be always connected, but this may
	 *            alter <b>numEdges</b>
	 */
	...
	public void getSimpleRandomTree(mxAnalysisGraph aGraph, int vertexCount) {
		...
		try {
			mxGraphStructure.makeTreeDirected(aGraph,
					vertices[(int) Math.round(Math.random() * (vertices.length - 1))]);
		} catch (StructuralException e) {
			log.log(Level.SEVERE, "Failed to generate a random tree graph", e);
		}
	};
	...
	/**
	 * Creates a new edge value based on graph properties in mxAnalysisGraph. Used
	 * mostly when creating new edges during graph generation.
	 * 
	 * @param aGraph
	 * @return
	 */
	 ...
	 * @param graph
	 * @param weighted
	 *            if true, the edges will be weighted, otherwise all will have
	 *            default value (1.0)
	 * @param minWeight
	 *            minimum edge weight if weighted
	 * @param maxWeight
	 *            maximum edge weight if weighted
	 * @return a generator function
	 */
	public static mxGeneratorFunction getGeneratorFunction(mxGraph graph, boolean weighted, double minWeight,
			double maxWeight) {
		if (weighted) {
	 ...
	 * @param graph
	 * @param forceConnected
	 *            if true, an unconnected graph is made connected
	 * @param forceSimple
	 *            if true, a non-simple graph is made simple Calculates one spanning
	 *            tree of graph, which doesn't have to be but can be minimal (this
	 *            is faster than minimal spanning tree, so if you need any spanning
	 *            tree, use this one) Self loops and multiple edges are
	 *            automatically removed! Also, unconnected graphs are made
	 *            connected!
	 * @throws StructuralException
	 *             the graph has to be simple (no self-loops and no multiple edges)
	 */
	public void oneSpanningTree(mxAnalysisGraph aGraph, boolean forceConnected, boolean forceSimple)
			throws StructuralException {
		mxGraph graph = aGraph.getGraph();
	...
	// TODO make a double check to avoid unnecessary cases of throwing an exception
	// (if the algorithm can't work out a solution, try a mirrored strategy)
	/**
	 * @param aGraph
	 * @param xDim
	 *            x dimension of the chessboard
	 * @param yDim
	 *            y dimension of the chessboard
	 * @param startVertexValue
	 *            vertex where the tour will start
	 * @throws StructuralException
	 *             not all size combinations are allowed, see wikipedia for a more
	 *             detailed explanation Returns a Knight's Tour graph
	 */
	public void getKnightTour(mxAnalysisGraph aGraph, int xDim, int yDim, int startVertexValue)
			throws StructuralException {
		if (xDim < 5 || yDim < 5) {
			throw new IllegalArgumentException();
		}
		...
		if (resultPath.size() < vertexNum) {
			...
			throw new StructuralException(
					"Could not generate a correct Knight tour with size " + xDim + " x " + yDim + ".");
		}
	};
	 ...
	 * @return
	 */
	private Object getNextKnightMove(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord, int yCoord,
			ArrayList<Object> resultPath) {
	...
	private int getPossibleKnightMoveCount(mxAnalysisGraph aGraph, int xDim, int yDim, int xCoord, int yCoord) {
		...
		if (currX > 0 && currX <= xDim && currY > 0 && currY <= yDim) {
			if (aGraph.getEdges(getVertexFromGrid(vertices, xDim, yDim, currX, currY), parent, false,
					true).length == 0) {
				possibleMoveCount++;
			}
		}

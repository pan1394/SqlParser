 ...
 * <h5>Prim</h5> <code>mga.getMinimumSpanningTree(graph, v, cf, true))</code>
 * <h5>Kruskal</h5> <code>mga.getMinimumSpanningTree(graph, v, e, cf))</code>
 * 
...
public class mxGraphAnalysis {
	 ...
	 * as an array of edges in order of traversal. <br>
	...
	/**
	 * Returns the minimum spanning tree (MST) for the graph defined by G=(E,V). The
	 * MST is defined as the set of all vertices with minimal lengths that forms no
	 * cycles in G.<br>
	...
	/**
	 * Returns the minimum spanning tree (MST) for the graph defined by G=(E,V). The
	 * MST is defined as the set of all vertices with minimal lenths that forms no
	 * cycles in G.<br>
	 ...
	 * O(|E|) find and O(|V|) union calls on the union find structure, thus yielding
	 * no more than O(|E|log|V|) steps. For a faster implementatin
	 * 
	...
	/**
	 * Returns a sorted set for <code>cells</code> with respect to <code>cf</code>.
	 * 
	 * @param states

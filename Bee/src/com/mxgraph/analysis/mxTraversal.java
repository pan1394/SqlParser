...
/**
 * Implements a collection of utility methods for traversing the graph
 * structure. This does not include tree traversal methods.
 */
public class mxTraversal {

	/**
	 * Implements a recursive depth first search starting from the specified cell.
	 * Process on the cell is performing by the visitor class passed in. The visitor
	 * has access to the current cell and the edge traversed to find this cell.
	 * Every cell is processed once only.
	 * 
	 * <pre>
	 ...
	 * @param visitor
	 */
	private static void dfsRec(mxAnalysisGraph aGraph, Object cell, Object edge, Set<Object> seen,
	 ...
	 * @param visitor
	 */
	private static void bfsRec(mxAnalysisGraph aGraph, Set<Object> queued, LinkedList<Object[]> queue,
			mxICellVisitor visitor) {
		if (queue.size() > 0) {
	...
	public static void dijkstra(mxAnalysisGraph aGraph, Object startVertex, Object endVertex, mxICellVisitor visitor)
		...
		if (!mxGraphStructure.isConnected(aGraph)) {
			throw new StructuralException(
					"The current Dijkstra algorithm only works for connected graphs and this graph isn't connected");
		}
		...
		while (vertexList.size() > 0) {
			...
			Object[] neighborVertices = aGraph.getOpposites(
					aGraph.getEdges(closestVertex, null, true, true, false, true), closestVertex, true, true);
	 ...
	 * @param aGraph
	 * @param startVertex
	 * @return a List where List(0) is the distance map and List(1) is the parent
	 *         map. See the example in GraphConfigDialog.java
	 * @throws StructuralException
	 *             - The Bellman-Ford algorithm only works for graphs without
	 *             negative cycles
	 */
	public static List<Map<Object, Object>> bellmanFord(mxAnalysisGraph aGraph, Object startVertex)
			throws StructuralException {
		...
		for (int i = 0; i < vertexNum; i++) {
			for (int j = 0; j < edgeNum; j++) {
				...
				double dist = (Double) distanceMap.get(source)
						+ costFunction.getCost(new mxCellState(view, currEdge, null));
				...
				if (!mxGraphProperties.isDirected(aGraph.getProperties(), mxGraphProperties.DEFAULT_DIRECTED)) {
					dist = (Double) distanceMap.get(target)
							+ costFunction.getCost(new mxCellState(view, currEdge, null));
		...
		for (int i = 0; i < edgeNum; i++) {
			...
			if (dist < (Double) distanceMap.get(target)) {
				throw new StructuralException(
						"The graph contains a negative cycle, so Bellman-Ford can't be completed.");
			}
		}
	...
	/**
	 * Implements the Floyd-Roy-Warshall (aka WFI) shortest path algorithm between
	 * all vertices.
	 * 
	 * @param aGraph
	 * @return an ArrayList where ArrayList(0) is the distance map and List(1) is
	 *         the path map. See the example in GraphConfigDialog.java
	 * @throws StructuralException
	 *             - The Floyd-Roy-Warshall algorithm only works for graphs without
	 *             negative cycles
	 */
	 ...
	 * @return
	 */
	private static Double[][] initializeWeight(mxAnalysisGraph aGraph, Object[] nodes, Object[] edges,
	...
	/**
	 * This method helps the user to get the desired data from the result of the
	 * Floyd-Roy-Warshall algorithm.
	 * 
	 * @param aGraph
	 * @param FWIresult
	 *            - the result of the Floyd-Roy-Warhall algorithm
	 * @param startVertex
	 * @param targetVertex
	 ...
	 *             negative cycles
	 */
	public static Object[] getWFIPath(mxAnalysisGraph aGraph, ArrayList<Object[][]> FWIresult, Object startVertex,
			Object targetVertex) throws StructuralException {
	 ...
	 * @throws StructuralException
	 */
	private static ArrayList<Object> getWFIPathRec(mxAnalysisGraph aGraph, Object[][] paths, Object startVertex,
			Object targetVertex, ArrayList<Object> currPath, mxCostFunction cf, mxGraphView view)
			throws StructuralException {

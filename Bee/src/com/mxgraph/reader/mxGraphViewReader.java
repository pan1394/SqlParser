...
public abstract class mxGraphViewReader extends DefaultHandler {
	...
	/**
	 * Holds the global scale of the graph. This is set just before createCanvas is
	 * called.
	 */
	...
	public void parseElement(String tagName, Map<String, Object> attrs) {
		if (canvas == null && tagName.equalsIgnoreCase("graph")) {
			...
			if ((edge && attrs.containsKey("points")) || ((vertex || group) && attrs.containsKey("x")
					&& attrs.containsKey("y") && attrs.containsKey("width") && attrs.containsKey("height"))) {
	...
	/**
	 * Parses the bounds, absolute points and label information from the style of
	 * the state into its respective fields and returns the label of the cell.
	 */
	public String parseState(mxCellState state, boolean edge) {
		...
		if (label != null && label.length() > 0) {
			...
			state.setLabelBounds(mxUtils.getLabelPaintBounds(label, state.getStyle(),
					mxUtils.isTrue(style, "html", false), offset, vertexBounds, scale));
		}

 ...
 * 
 * <pre>
 * mxGraphics2DCanvas.putTextShape(mxGraphics2DCanvas.TEXT_SHAPE_HTML, new mxHtmlTextShape() {
 * 	protected String createHtmlDocument(Map<String, Object> style, String text) {
 * 		return mxUtils.createHtmlDocument(style, text, 1, 0,
 * 				"<style type=\"text/css\">.selectRef { " + "font-size:9px;font-weight:normal; }</style>");
 * 	}
 * });
 * </pre>
 */
public class mxHtmlTextShape implements mxITextShape {
	...
	/**
	 * Specifies if linefeeds should be replaced with breaks in HTML markup. Default
	 * is true.
	 */
	...
	public void paintShape(mxGraphics2DCanvas canvas, String text, mxCellState state, Map<String, Object> style) {
		...
		if (textRenderer != null && rendererPane != null
			...
			textRenderer.setText(createHtmlDocument(style, text, (int) Math.round(w / state.getView().getScale()),

...
public class mxConnectorShape extends mxBasicShape {
	...
	public mxPoint paintMarker(mxGraphics2DCanvas canvas, mxCellState state, boolean source) {
		...
		String type = mxUtils.getString(style, (source) ? mxConstants.STYLE_STARTARROW : mxConstants.STYLE_ENDARROW,
				"");
		float size = (mxUtils.getFloat(style, (source) ? mxConstants.STYLE_STARTSIZE : mxConstants.STYLE_ENDSIZE,
	...
	/**
	 * Hook to override creation of the vector that the marker is drawn along since
	 * it may not be the same as the vector between any two control points
	 * 
	 * @param points

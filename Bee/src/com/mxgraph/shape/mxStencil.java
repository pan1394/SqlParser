...
public class mxStencil implements mxIShape {
		...
		if (strokeColor != null || fillColor != null) {
			...
			if (fillColor != null) {
				if (gradientColor != null && !gradientColor.equals("transparent")) {
					canvas.setGradient(fillColor, gradientColor, state.getX(), state.getY(), state.getWidth(),
							state.getHeight(), direction, 1, 1);
				} else {
	...
	protected void drawShadow(mxGraphicsCanvas2D canvas, mxCellState state, double rotation, boolean flipH,
		...
		mxPoint offset = mxUtils.getRotatedPoint(new mxPoint(mxConstants.SHADOW_OFFSETX, mxConstants.SHADOW_OFFSETY),
				cos, sin);
		...
		// TODO: Use save/restore instead of negative offset to restore (requires fix
		// for HTML canvas)
	 ...
	 */
	public boolean drawShape(mxGraphicsCanvas2D canvas, mxCellState state, mxRectangle bounds, mxRectangle aspect,
			boolean background) {
	...
	protected void drawElement(mxGraphicsCanvas2D canvas, mxCellState state, Element node, mxRectangle aspect) {
		...
		if (name.equals("save")) {
			...
			canvas.curveTo(x0 + getDouble(node, "x1") * sx, y0 + getDouble(node, "y1") * sy,
					x0 + getDouble(node, "x2") * sx, y0 + getDouble(node, "y2") * sy, lastMoveX, lastMoveY);
			...
			double[] curves = mxUtils.arcToCurves(this.lastMoveX, this.lastMoveY, r1, r2, angle, largeArcFlag,
					sweepFlag, x, y);
			...
			canvas.rect(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, getDouble(node, "w") * sx,
					getDouble(node, "h") * sy);
			...
			canvas.roundrect(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, getDouble(node, "w") * sx,
					getDouble(node, "h") * sy, r, r);
		} else if (name.equals("ellipse")) {
			canvas.ellipse(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, getDouble(node, "w") * sx,
					getDouble(node, "h") * sy);
		} else if (name.equals("image")) {
			String src = evaluateAttribute(node, "src", state);

			canvas.image(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, getDouble(node, "w") * sx,
					getDouble(node, "h") * sy, src, false, getString(node, "flipH", "0").equals("1"),
			...
			canvas.text(x0 + getDouble(node, "x") * sx, y0 + getDouble(node, "y") * sy, 0, 0, str,
					node.getAttribute("align"), node.getAttribute("valign"), false, "", null, false, rotation, null);
	 ...
	 * does not exist then the text content of the node is evaluated and if it is a
	 * function it is invoked with <state> as the only argument and the return value
	 * is used as the attribute value to be returned.
	 */

...
/**
 * Generic codec for Java objects. See below for a detailed description of the
...
public class mxObjectCodec {
	...
	/**
	 * Constructs a new codec for the specified template object. The variables in
	 * the optional exclude array are ignored by the codec. Variables in the
	 * optional idrefs array are turned into references in the XML. The optional
	 * mapping may be used to map from variable names to XML attributes. The
	 * argument is created as follows:
	 * 
	 * @param template
	 ...
	 * 
	 * @param node
	 *            XML node that the object is going to represent.
	 ...
	 * implementation returns true if the given fieldname is in {@link #exclude}.
	 * 
	 * @param obj
	 ...
	 *            Value of the field.
	 * @param write
	 *            Boolean indicating if the field is being encoded or decoded. write
	 *            is true if the field is being encoded, else it is being decoded.
	...
	/**
	 * Returns true if the given fieldname is to be treated as a textual reference
	 * (ID). This implementation returns true if the given fieldname is in
	 * {@link #idrefs}.
	 * 
	 * @param obj
	 *            Object instance that contains the field.
	 ...
	 *            Value of the field.
	 * @param isWrite
	 *            Boolean indicating if the field is being encoded or decoded.
	 *            isWrite is true if the field is being encoded, else it is being
	 *            decoded.
	 ...
	 * object. Calls beforeEncode after creating the node and afterEncode with the
	 ...
	 * {@link mxCodec#getId(Object)} is used to replace the object with its ID.</li>
	 * <li>The variable name is mapped using {@link #mapping}.</li>
	 * <li>If obj is an array and the variable name is numeric (ie. an index) then
	 * it is not encoded.</li>
	 * <li>If the value is an object, then the codec is used to create a child node
	 * with the variable name encoded into the "as" attribute.</li>
	 * <li>Else, if {@link com.mxgraph.io.mxCodec#isEncodeDefaults()} is true or the
	 * value differs from the template value, then ...
	 * <ul>
	 * <li>... if obj is not an array, then the value is mapped to an
	 * attribute.</li>
	 * <li>... else if obj is an array, the value is mapped to an add child with a
	 * value attribute or a text child node, if the value is a function.</li>
	 * </ul>
	 * </li>
	 * </ul>
	 * 
	 ...
	 * {@link #encodeFields(mxCodec, Object, Node)} and
	 * {@link #encodeElements(mxCodec, Object, Node)}.
	 * 
	 * @param enc
	 *            Codec that controls the encoding process.
	 * @param obj
	 ...
	 *            Codec that controls the encoding process.
	 * @param obj
	 *            Object whose fields should be encoded.
	 * @param node
	 *            XML node that contains the encoded object.
	 */
	 ...
	 *            Codec that controls the encoding process.
	 * @param obj
	 *            Object whose child objects should be encoded.
	 * @param node
	 *            XML node that contains the encoded object.
	 */
	 ...
	 * and uses {@link #writeAttribute(mxCodec, Object, String, Object, Node)} to
	 * write the attribute into the given node.
	 * 
	 * @param enc
	 *            Codec that controls the encoding process.
	 * @param obj
	...
	protected void encodeValue(mxCodec enc, Object obj, String fieldname, Object value, Node node) {
		if (value != null && !isExcluded(obj, fieldname, value, true)) {
			if (isReference(obj, fieldname, value, true)) {
				...
				if (tmp == null) {
					log.severe("mxObjectCodec.encode: No ID for " + getName() + "." + fieldname + "=" + value);
					return; // exit
				}
	...
	protected boolean isPrimitiveValue(Object value) {
		return value instanceof String || value instanceof Boolean || value instanceof Character
				...
				|| value instanceof Float || value instanceof Double || value.getClass().isPrimitive();
	}
	...
	protected void writeComplexAttribute(mxCodec enc, Object obj, String attr, Object value, Node node) {
		...
		if (child != null) {
			...
			log.severe("mxObjectCodec.encode: No node for " + getName() + "." + attr + ": " + value);
		}
	}
	...
	/**
	 * Returns the XML node attribute name for the given Java field name. That is,
	 * it returns the mapping of the field name.
	 */
	...
	/**
	 * Returns the value of the field using the accessor for the field if one
	 * exists.
	 */
	...
	/**
	 * Sets the value of the field with the specified name in the specified object
	 * instance.
	 */
	 ...
	 * encoding and return the post-processed node. This implementation returns the
	 * input node. The return value of this method is returned to the encoder from
	 * <encode>.
	 * 
	 * Parameters:
	 * 
	...
	/**
	 * Parses the given node into the object or returns a new object representing
	 * the given node.
	 * 
	 * @param dec
	 *            Codec that controls the encoding process.
	 ...
	 * object. If the object is not yet in the cache then it is constructed using
	 * the constructor of <template> and cached in <mxCodec.objects>.
	 * 
	 * This implementation decodes all attributes and childs of a node according to
	 * the following rules: - If the variable name is in <exclude> or if the
	 * attribute name is "id" or "as" then it is ignored. - If the variable name is
	 * in <idrefs> then <mxCodec.getObject> is used to replace the reference with an
	 * object. - The variable name is mapped using a reverse <mapping>. - If the
	 * value has a child node, then the codec is used to create a child object with
	 * the variable name taken from the "as" attribute. - If the object is an array
	 * and the variable name is empty then the value or child object is appended to
	 * the array. - If an add child has no value or the object is not an array then
	 ...
	 * @return Returns the resulting object that represents the given XML node or
	 *         the object given to the method as the into parameter.
	 */
	...
	protected void decodeAttribute(mxCodec dec, Node attr, Object obj) {
		...
		if (!name.equalsIgnoreCase("as") && !name.equalsIgnoreCase("id")) {
			...
			if (isReference(obj, fieldname, value, false)) {
				...
				if (tmp == null) {
					log.severe("mxObjectCodec.decode: No object for " + getName() + "." + fieldname + "=" + value);
					return; // exit
				}
	 ...
	 * is a collection. The value is then used to populate the field for a new
	 * instance. For strongly typed languages it may be required to override this to
	 * return the correct collection instance based on the encoded child.
	 */
	...
	/**
	 * Sets the decoded child node as a value of the given object. If the object is
	 * a map, then the value is added with the given fieldname as a key. If the
	 * fieldname is not empty, then setFieldValue is called or else, if the object
	 * is a collection, the value is added to the collection. For strongly typed
	 * languages it may be required to override this with the correct code to add an
	 * entry to an object.
	 */
	 ...
	 * include by decoding the XML document. Returns false if the given node is not
	 ...
	 * return the node to be used for further processing by
	 * {@link #decode(mxCodec, Node)}. The object is created based on the template
	 * in the calling method and is never null.
	 * 
	 * This implementation returns the input node. The return value of this function
	 * is used in {@link #decode(mxCodec, Node)} to perform the default decoding
	 ...
	 * implementation returns the given object without any changes. The return value
	 * of this method is returned to the decoder from

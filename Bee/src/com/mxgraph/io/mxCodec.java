...
/**
 * XML codec for Java object graphs. In order to resolve forward references when
 * reading files the XML document that contains the data must be passed to the
 * constructor.
 */
public class mxCodec {
	 ...
	 * 
	 * @param document
	 *            Optional XML document that contains the data. If no document is
	 *            specified then a new document is created using
	 ...
	 * {@link #document}. If the object is not known then {@link #lookup(String)} is
	 * used to find an object. If no object is found, then the element with the
	 * respective ID from the document is parsed using {@link #decode(Node)}.
	 * 
	 * @param id
	 ...
	 * existing object to be used. If no object is given, then a new instance is
	 * created using the constructor from the codec.
	 * 
	 * The function returns the passed in object or the new instance if no object
	 * was given.
	 * 
	 * @param node
	 *            XML node to be decoded.
	 ...
	 * into the given node. The children are not encoded if the optional
	 * includeChildren is false. The function is in charge of adding the result into
	 * the given node and has no return value.
	 * 
	 * @param cell
	...
	/**
	 * Decodes cells that have been encoded using inversion, ie. where the user
	 * object is the enclosing node in the XML, and restores the group and graph
	 * structure in the cells. Returns a new <mxCell> instance that represents the
	 * given node.
	 * 
	 * @param node
	 *            XML node that contains the cell data.
	 * @param restoreStructures
	 *            Boolean indicating whether the graph structure should be restored
	 *            by calling insert and insertEdge on the parent and terminals,
	 *            respectively.

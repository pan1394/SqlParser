 ...
 * wrapper around the cells which are in charge of storing the actual graph
 * datastructure. The model acts as a transactional wrapper with event
 * notification for all changes, whereas the cells contain the atomic operations
 * for updating the actual datastructure.
 * 
 * Layers:
 * 
 ...
 * <code>edit</code> property contains the mxUndoableEdit. The
 * <code>changes</code> property contains the list of undoable changes inside
 * the undoable edit. The changes property is deprecated, please use
 * edit.getChanges() instead.
 * 
 * mxEvent.EXECUTE fires between begin- and endUpdate and after an atomic change
 * was executed in the model. The <code>change</code> property contains the
 * atomic change that was executed.
 * 
 * mxEvent.BEGIN_UPDATE fires after the updateLevel was incremented in
 * beginUpdate. This event contains no properties.
 * 
 * mxEvent.END_UPDATE fires after the updateLevel was decreased in endUpdate but
 * before any notification or change dispatching. The <code>edit</code> property
 * contains the current mxUndoableEdit.
 * 
 * mxEvent.BEFORE_UNDO fires before the change is dispatched after the update
 * level has reached 0 in endUpdate. The <code>edit</code> property contains the
 * current mxUndoableEdit.
 * 
...
public class mxGraphModel extends mxEventSource implements mxIGraphModel, Serializable {
	 ...
	 * then a new object is created for this variable using createUndoableEdit.
	 */
	...
	/**
	 * Returns the cell for the specified Id or null if no cell can be found for the
	 * given Id.
	 * 
	 * @param id
	...
	/**
	 * Returns true if the model automatically update parents of edges so that the
	 * edge is contained in the nearest-common-ancestor of its terminals.
	 * 
	...
	/**
	 * Specifies if the model automatically updates parents of edges so that the
	 * edge is contained in the nearest-common-ancestor of its terminals.
	 * 
	 * @param maintainEdgeParent
	...
	/**
	 * Specifies if the model automatically creates Ids for new cells and resolves
	 * Id collisions.
	 * 
	 * @param value
	...
	/**
	 * Invoked after a cell has been added to a parent. This recursively creates an
	 * Id for the new cell and/or resolves Id collisions.
	 * 
	 * @param cell
	...
	/**
	 * Creates a new Id for the given cell and increments the global counter for
	 * creating new Ids.
	 * 
	 * @param cell
	 ...
	 * removes the cell from its terminals and removes the mapping from the Id to
	 * the cell.
	 * 
	 * @param cell
	...
	/**
	 * Inner callback to update the parent of a cell using mxCell.insert on the
	 ...
	 * mxCell.setValue and return the previous value, that is, the return value of
	 * mxCell.getValue.
	 */
	...
	/**
	 * Inner callback to update the style of the given mxCell using mxCell.setStyle
	 * and return the previous style.
	 */
	...
	/**
	 * Sets the visible state of the given mxCell using mxVisibleChange and adds the
	 ...
	 * have no identity and are always cloned unless the cloneAllEdges flag is set
	 * to false, in which case edges with the same id in the target model are
	 ...
	 * target cell with the same id or the clone of the source cell that was
	 ...
	 * @param outgoing
	 *            Boolean that specifies if the number of outgoing or incoming edges
	 *            should be returned.
	 ...
	 * 
	 * @param model
	 *            Graphmodel that contains the edges.
	 * @param cell
	 ...
	 * @return Returns the array of connected edges for the given cell.
	 */
	public static Object[] getEdges(mxIGraphModel model, Object cell, boolean incoming, boolean outgoing,
	 ...
	 * @param source
	 *            Object that defines the source cell.
	 * @param target
	 *            Object that defines the target cell.
	 ...
	 *            Object that defines the target cell.
	 * @param directed
	 *            Boolean that specifies if the direction of the edge should be
	 *            taken into account.
	 ...
	 * returning sources and/or targets as specified. The result is returned as an
	 * array of mxCells.
	 * 
	 * @param model
	 *            Model that contains the connection information.
	 ...
	 *            Cell that specifies the known end of the edges.
	 * @param sources
	 *            Boolean that specifies if source terminals should be contained in
	 *            the result. Default is true.
	 * @param targets
	 *            Boolean that specifies if target terminals should be contained in
	 *            the result. Default is true.
	 * @return Returns the array of opposite terminals for the given edges.
	 */
	public static Object[] getOpposites(mxIGraphModel model, Object[] edges, Object terminal, boolean sources,
			boolean targets) {
	 ...
	 * depending on the arguments. If both arguments are false then all children are
	...
	/**
	 * Returns a all descendants of the given cell and the cell itself as a
	 * collection.
	 */
	 ...
	 * desceandants for each <mxCell> that it contains. Duplicates should be removed
	 * in the cells array to improve performance.
	 * 
	 * Parameters:
	 * 
	...
	public static class mxChildChange extends mxAtomicGraphModelChange {
		...
		/**
		 * Gets the source or target terminal field for the given edge even if the edge
		 * is not stored as an incoming or outgoing edge in the respective terminal.
		 */
		...
		/**
		 * Sets the source or target terminal field for the given edge without inserting
		 * an incoming or outgoing edge in the respective terminal.
		 */
		...
		protected int getChildIndex(Object parent, Object child) {
			return (parent instanceof mxICell && child instanceof mxICell)
					? ((mxICell) parent).getIndex((mxICell) child)
					: 0;
		}

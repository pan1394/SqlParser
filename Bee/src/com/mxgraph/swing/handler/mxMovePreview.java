...
public class mxMovePreview extends mxEventSource {
	...
	/**
	 * Specifies if the preview should use clones of the original shapes. Default is
	 * true.
	 */
	 ...
	 * Default is true. This should not be used if cloning is enabled.
	 */
	protected boolean contextPreview = true;

	/**
	 * Specifies if the selection cells handler should be hidden while the preview
	 * is visible. Default is false.
	 */
	...
	public void start(MouseEvent e, mxCellState state) {
		...
		fireEvent(new mxEventObject(mxEvent.START, "event", e, "state", startState));
	}
	...
	public Object[] stop(boolean commit, MouseEvent e, double dx, double dy, boolean clone, Object target) {
		...
		try {
			...
			fireEvent(new mxEventObject(mxEvent.STOP, "event", e, "commit", commit));
		} finally {
	...
	public void paint(Graphics g) {
		if (placeholder != null) {
			mxSwingConstants.PREVIEW_BORDER.paintBorder(graphComponent, g, placeholder.x, placeholder.y,

...
public class mxConnectionHandler extends mxMouseAdapter {
	 ...
	 * Default is mxConstants.CONNECT_HANDLE_SIZE.
	 */
	protected int handleSize = mxConstants.CONNECT_HANDLE_SIZE;

	/**
	 * Specifies if a handle should be used for creating new connections. This is
	 * only used if no connectIcon is specified. If this is false, then the source
	 * cell will be highlighted when the mouse is over the hotspot given in the
	 * marker. Default is mxConstants.CONNECT_HANDLE_ENABLED.
	 */
	...
	/**
	 * Specifies if the source should be cloned and used as a target if no target
	 * was selected. Default is false.
	 */
	...
	/**
	 * Returns true if the source terminal has been clicked and a new connection is
	 ...
	 * mxGraph.getEdgeValidationError in validateConnection. This is an additional
	 * hook for disabling certain targets in this specific handler.
	 */
	 ...
	 * source target pair is not valid. Otherwise it returns null.
	 */
	public String validateConnection(Object source, Object target) {
		...
		return graphComponent.getGraph().getEdgeValidationError(connectPreview.getPreviewState().getCell(), source,
				target);
	}
	...
	public void mousePressed(MouseEvent e) {
		if (!graphComponent.isForceMarqueeEvent(e) && !graphComponent.isPanningEvent(e) && !e.isPopupTrigger()
				...
				&& ((isHighlighting() && marker.hasValidState())
						|| (!isHighlighting() && bounds != null && bounds.contains(e.getPoint())))) {
					...
					try {
						...
						if (cell != null) {
							...
							eventSource.fireEvent(
									new mxEventObject(mxEvent.CONNECT, "cell", cell, "event", e, "target", dropTarget));
						}
	...
	public void setBounds(Rectangle value) {
		if ((bounds == null && value != null) || (bounds != null && value == null)

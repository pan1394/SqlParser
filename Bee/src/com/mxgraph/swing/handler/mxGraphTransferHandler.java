...
public class mxGraphTransferHandler extends TransferHandler {
	...
	/**
	 * Boolean that specifies if an image of the cells should be created for each
	 * transferable. Default is true.
	 */
	public static boolean DEFAULT_TRANSFER_IMAGE_ENABLED = true;

	/**
	 * Specifies the background color of the transfer image. If no color is given
	 * here then the background color of the enclosing graph component is used.
	...
	/**
	 * Specifies if a transfer image should be created for the transferable. Default
	 * is DEFAULT_TRANSFER_IMAGE.
	 */
		...
		if (c instanceof mxGraphComponent) {
			...
			if (!graph.isSelectionEmpty()) {
				...
				if (originalCells.length > 0) {
					ImageIcon icon = (transferImageEnabled) ? createTransferableImage(graphComponent, originalCells)
							: null;
	...
	public ImageIcon createTransferableImage(mxGraphComponent graphComponent, Object[] cells) {
		...
		Image img = mxCellRenderer.createBufferedImage(graphComponent.getGraph(), cells, 1, bg,
	...
	public void exportDone(JComponent c, Transferable data, int action) {
		...
		if (c instanceof mxGraphComponent && data instanceof mxGraphTransferable) {
			// Requires that the graph handler resets the location to null if the drag
			// leaves the
	 ...
	 * mxGraph.isSplitTarget. Selects and returns the cells that have been imported.
	 */

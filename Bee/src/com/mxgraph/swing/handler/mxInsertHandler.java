...
public class mxInsertHandler extends mxMouseAdapter {
	...
	public Object insertCell(mxRectangle bounds) {
		...
		return graphComponent.getGraph().insertVertex(null, null, "", bounds.getX(), bounds.getY(), bounds.getWidth(),
	...
	public void reset() {
		...
		if (dirty != null) {
			int b = (int) Math.ceil(lineWidth);
			graphComponent.getGraphControl().repaint(dirty.x - b, dirty.y - b, dirty.width + 2 * b,
					dirty.height + 2 * b);
		}
	}

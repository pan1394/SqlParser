 ...
 * under the mouse. The <code>state</code> property contains the mxCellState of
 * the respective cell or null if no cell is under the mouse.
 * 
 * To create a cell marker which highlights cells "in-place", the following code
 * can be used: <code>
...
public class mxCellMarker extends JComponent {
	...
	/**
	 * Specifies the portion of the width and height that should trigger a
	 * highlight. The area around the center of the cell to be marked is used as the
	 * hotspot. Possible values are between 0 and 1. Default is
	...
	/**
	 * Specifies if the the content area of swimlane should be non-transparent to
	 * mouse events. Default is false.
	 */
	...
	/**
	 * Returns true if the marker is enabled, that is, if it processes events in
	 * process.
	 */
	...
	/**
	 * Processes the given event and marks the state returned by getStateAt with the
	 * color returned by getMarkerColor. If the markerColor is not null, then the
	 * state is stored in markedState. If isValidState returns true, then the state
	 * is stored in validState regardless of the marker color. The state is returned
	 * regardless of the marker color and valid state.
	 */
	public mxCellState process(MouseEvent e) {
		mxCellState state = null;
	...
	/**
	 * Returns true if the given state is a valid state. If this returns true, then
	 * the state is stored in validState. The return value of this method is used as
	 * the argument for getMarkerColor.
	 */
	...
	/**
	 * Uses getCell, getMarkedState and intersects to return the state for the given
	 * event.
	 */
	 ...
	 * returns true if the hotspot is 0 or the event is inside the hotspot for the
	 * given cell state.
	 */
	protected boolean intersects(mxCellState state, MouseEvent e) {
		if (isHotspotEnabled()) {
			return mxUtils.intersectsHotspot(state, e.getX(), e.getY(), hotspot, mxConstants.MIN_HOTSPOT_SIZE,

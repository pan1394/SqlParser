...
public class mxInteractiveCanvas extends mxGraphics2DCanvas {
	...
	public boolean contains(mxGraphComponent graphComponent, Rectangle rect, mxCellState state) {
		return state != null && state.getX() >= rect.x && state.getY() >= rect.y
	 ...
	 * implementation does not check if the given state is a swimlane, it is assumed
	 * that the caller has checked this before using this method.
	 */

...
public class mxCellStatePreview {
	...
	/**
	 * Constructs a new state preview. The paint handler to invoke the paint method
	 * must be installed elsewhere.
	 */
	...
	protected mxRectangle revalidateState(mxCellState parentState, mxCellState state, double dx, double dy) {
		...
		if (state != null) {
			...
			if ((dx != 0 || dy != 0) && geo != null && geo.isRelative() && model.isVertex(cell)
					&& (parentState == null || model.isVertex(parentState.getCell()) || deltas.get(state) != null)) {
				state.setX(state.getX() + dx);
				state.setY(state.getY() + dy);
	...
	protected void paintPreviewState(mxGraphics2DCanvas canvas, mxCellState state) {
		graphComponent.getGraph().drawState(canvas, state,
				state.getCell() != graphComponent.getCellEditor().getEditingCell());
	}
}

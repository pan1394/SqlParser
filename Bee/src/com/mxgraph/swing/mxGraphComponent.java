...
public class mxGraphComponent extends JScrollPane implements Printable {
	...
	/**
	 * Specifies the <mxImage> to be returned by <getBackgroundImage>. Default is
	 * null.
	 */
	...
	/**
	 * If the pageFormat should be used to determine the minimal graph bounds even
	 * if the page is not visible (see pageVisible). Default is false.
	 */
	...
	/**
	 * Specifies if the background page should be centered by automatically setting
	 * the translate in the view. Default is true. This does only apply if
	 * pageVisible is true.
	 */
	...
	/**
	 * Whether or not to scroll the scrollable container the graph exists in if a
	 * suitable handler is active and the graph bounds already exist extended in the
	 * direction of mouse travel.
	 */
	protected boolean autoScroll = true;

	/**
	 * Whether to extend the graph bounds and scroll towards the limit of those new
	 * bounds in the direction of mouse travel if a handler is active while the
	 * mouse leaves the container that the graph exists in.
	 */
	...
	/**
	 * Specifies <escape> should be invoked when the escape key is pressed. Default
	 * is true.
	 */
	 ...
	 * diagram changing or other means stopCellEditing is invoked, and changes are
	 * saved. This is implemented in a mouse listener in this class. Default is
	 * true.
	 */
	protected boolean invokesStopCellEditing = true;

	/**
	 * If true, pressing the enter key without pressing control will stop editing
	 * and accept the new value. This is used in <mxKeyHandler> to stop cell
	 * editing. Default is false.
	 */
	protected boolean enterStopsCellEditing = false;

	/**
	 * Specifies the zoom policy. Default is ZOOM_POLICY_PAGE. The zoom policy does
	 * only apply if pageVisible is true.
	 */
	...
	/**
	 * Adds handling of edit and stop-edit events after all other handlers have been
	 * installed.
	 */
	...
	/**
	 * Creates the inner control that handles tooltips, preferred size and can draw
	 * cells onto a canvas.
	 */
	...
	/**
	 * Clones and inserts the given cells into the graph using the move method and
	 * returns the inserted cells. This shortcut is used if cells are inserted via
	 * datatransfer.
	 */
	...
	public void startEditingAtCell(Object cell, EventObject evt) {
		...
		if (cell != null) {
			eventSource.fireEvent(new mxEventObject(mxEvent.START_EDITING, "cell", cell, "event", evt));
	...
	/**
	 * Returns the (unscaled) preferred size for the current page format (scaled by
	 * pageScale).
	 */
	...
	/**
	 * Invoked after the component was resized to update the zoom if the zoom policy
	 * is not none and/or update the translation of the diagram if pageVisible and
	 * centerPage are true.
	 */
	...
	public void zoomActual() {
		mxPoint translate = (pageVisible && centerPage) ? getPageTranslate(1) : new mxPoint();
	 ...
	 * cell hierarchy starting at the given parent.
	 * 
	 * @param x
	 ...
	 * @param parent
	 *            <mxCell> that should be used as the root of the recursion. Default
	 *            is <defaultParent>.
				...
				for (int i = childCount - 1; i >= 0; i--) {
					...
					if (result != null) {
						...
						if (state != null && canvas.intersects(this, hit, state) && (!graph.isSwimlane(cell)
								|| hitSwimlaneContent
								|| (transparentSwimlaneContent && !canvas.hitSwimlaneContent(this, state, x, y)))) {
							return cell;
						}
					}
				}
	 ...
	 * rectangle (x, y, width, height). The result is added to the optional result
	 * array, which is returned from the function. If no result array is specified
	 * then a new array is created and returned.
	 * 
	...
	/**
	 * Returns true if the absolute value of one of the given parameters is greater
	 * than the tolerance.
	 */
	...
	/**
	 * Returns the icon used to display the collapsed state of the specified cell
	...
	/**
	 * Returns true if the given cell can be imported via datatransfer. This returns
	 * importEnabled.
	 */
	...
	/**
	 * Note: This is not used during drag and drop operations due to limitations of
	 * the underlying API. To enable this for move operations set dragEnabled to
	 * false.
	 * 
	 * @param event
	 ...
	 * printing. This must be overridden to return a custom canvas if there are any
	 * custom shapes.
	 */
	...
	public mxCellHandler createHandler(mxCellState state) {
		if (graph.getModel().isVertex(state.getCell())) {
			...
			if (graph.isLoop(state) || style == mxEdgeStyle.ElbowConnector || style == mxEdgeStyle.SideToSide
	 ...
	 * root of the model. Context is an object that contains the validation state
	 * for the complete validation run. The validation errors are attached to their
	 * cells using <setWarning>. This function returns true if no validation errors
	 * exist in the graph.
	 */
	public String validateGraph() {
		return validateGraph(graph.getModel().getRoot(), new Hashtable<Object, Object>());
	}
	...
	/**
	 * Adds an overlay for the specified cell. This method fires an addoverlay event
	 * and returns the new overlay.
	 * 
	 * @param cell
	...
	/**
	 * Returns the array of overlays for the given cell or null, if no overlays are
	 * defined.
	 * 
	 * @param cell
	...
	/**
	 * Removes and returns the given overlay from the given cell. This method fires
	 * a remove overlay event. If no overlay is given, then all overlays are removed
	...
	public mxICellOverlay removeCellOverlay(Object cell, mxICellOverlay overlay) {
		if (overlay == null) {
			...
			if (arr != null) {
				...
				List<mxICellOverlay> list = new ArrayList<mxICellOverlay>(Arrays.asList(arr));
	...
	/**
	 * Removes all overlays from the given cell. This method fires a removeoverlay
	 * event for each removed overlay and returns the array of overlays that was
	...
	/**
	 * Notified when an overlay has been removed from the graph. This implementation
	 * removes the given overlay from its parent if it is a component inside a
	 * component hierarchy.
	 */
	 ...
	 * If no cell is specified then all overlays are removed from the graph. This
	 * implementation uses removeOverlays to remove the overlays from the individual
	 * cells.
	 * 
	 * @param cell
	 *            Optional cell that represents the root of the subtree to remove
	 ...
	 * warningImage and returns the new overlay. If the warning is null or a zero
	 * length string, then all overlays are removed from the cell instead.
	 * 
	 * @param cell
	...
	/**
	 * Returns a hashtable with all entries from the overlays variable where a cell
	 * still exists in the model. The entries are removed from the global hashtable
	 * so that the remaining entries reflect those whose cell have been removed from
	 * the model. If no state is available for a given cell then its overlays are
	 * temporarly removed from the rendering control, but kept in the result.
	 */
	...
	protected Rectangle paintBackgroundPage(Graphics g) {
		...
		if (isPageBreaksVisible() && (horizontalPageCount > 1 || verticalPageCount > 1)) {
			...
			g2.setStroke(
					new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, new float[] { 1, 2 }, 0));
		...
		if (isGridVisible()) {
			...
			case GRID_STYLE_DASHED: {
				...
				Stroke[] strokes = new Stroke[] {
						new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1, new float[] { 3, 1 },
								Math.max(0, iys) % 4),
						new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1, new float[] { 2, 2 },
								Math.max(0, iys) % 4),
	...
	/**
	 * Checks if the triple buffer exists and creates a new one if it does not. Also
	 * compares the size of the buffer with the size of the graph and drops the
	 * buffer if it has a different size.
	 */
	...
	/**
	 * Creates the tripleBufferGraphics and tripleBuffer for the given dimension and
	 * draws the complete graph onto the triplebuffer.
	 * 
	 * @param width
	 * @param height
	 */
	...
	/**
	 * Clears and repaints the triple buffer at the given rectangle or repaints the
	 * complete buffer if no rectangle is specified.
	 * 
	 * @param dirty
	 */
	public void repaintTripleBuffer(Rectangle dirty) {
		if (tripleBuffered && tripleBufferGraphics != null) {
			if (dirty == null) {
				dirty = new Rectangle(tripleBuffer.getWidth(), tripleBuffer.getHeight());
			}
	...
	public class mxGraphControl extends JComponent {
		...
		/**
		 * Specifies a translation for painting. This should only be used during mouse
		 * drags and must be reset after any interactive repaints. Default is (0,0).
		...
		/**
		 * Overrides parent method to add extend flag for making the control larger
		 * during previews.
		 */
		...
		/**
		 * Implements extension of the component in all directions. For extension below
		 * the origin (into negative space) the translate will temporaly be used and
		 * reset with the next mouse released event.
		 */
		...
		public void paint(Graphics g) {
			...
			eventSource.fireEvent(new mxEventObject(mxEvent.BEFORE_PAINT, "g", g));
			super.paint(g);
			eventSource.fireEvent(new mxEventObject(mxEvent.AFTER_PAINT, "g", g));
			g.translate(-translate.x, -translate.y);
		}
		...
		/**
		 * @param state
		 *            the cached state of the cell whose extended bounds are to be
		 *            calculated
		 * @return the bounds of the cell, including the label and shadow and allowing
		 *         for rotation
		 */
		protected Rectangle getExtendedCellBounds(mxCellState state) {
			...
			int border = (int) Math.ceil(
					mxUtils.getDouble(state.getStyle(), mxConstants.STYLE_STROKEWIDTH) * graph.getView().getScale())
					+ 1;
			tmp.grow(border);
		...
		/**
		 * Draws the given cell onto the specified canvas. This is a modified version of
		 * mxGraph.drawCell which paints the label only if the corresponding cell is not
		 * being edited and invokes the cellDrawn hook after all descendants have been
		 * painted.
		 * 
		 * @param canvas
		...
		public void drawCell(mxICanvas canvas, Object cell) {
			...
			if (state != null && isCellDisplayable(state.getCell())
					&& (!(canvas instanceof mxGraphics2DCanvas) || hitClip((mxGraphics2DCanvas) canvas, state))) {
		...
		/**
		 * Draws the child edges and/or all other children in the given cell depending
		 * on the boolean arguments.
		 */
		...
		/**
		 * Returns true if the given cell is not the current root or the root in the
		 * model. This can be overridden to not render certain cells in the graph
		 * display.
		 */

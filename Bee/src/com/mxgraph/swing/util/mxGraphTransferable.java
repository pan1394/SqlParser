...
public class mxGraphTransferable implements Transferable, UIResource, Serializable {
	...
	/**
	 * Global switch to disable image support in transferables. Set this to false as
	 * a workaround for Data translation failed: not an image format in Java 1.7 on
	 * Mac OS X.
	 */
	public static boolean enableImageSupport = true;

	/**
	 * Serialized Data Flavor. Use the following code to switch to local reference
	 * flavor: <code>
	 * try
	 * {
	 ...
	 * plain text. If this method returns a non-null value, it will be placed at the
	...
	/**
	 * Whether the HTML flavors are offered. If so, the method getHTMLData should be
	...
	/**
	 * Whether the plain text flavors are offered. If so, the method getPlainData
	...
	static {
		...
		try {
			dataFlavor = new DataFlavor(
					DataFlavor.javaSerializedObjectMimeType + "; class=com.mxgraph.swing.util.mxGraphTransferable");
		} catch (ClassNotFoundException e) {

...
public class mxGraphOutline extends JComponent {
	...
	/**
	 * Specifies if the outline should be zoomed to the page if the graph component
	 * is in page layout mode. Default is true.
	 */
	...
	/**
	 * Paints the foreground. Foreground is dynamic and should never be made part of
	 * the triple buffer. It is painted on top of the buffer.
	 */
	...
	public boolean updateScaleAndTranslate() {
		...
		if (this.graphComponent != null) {
			...
			if (gw > 0 && gh > 0) {
				boolean magnifyPage = graphComponent.isPageVisible() && isFitPage()
	...
	public class MouseTracker implements MouseListener, MouseMotionListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (graphComponent != null && !e.isConsumed() && !e.isPopupTrigger()
		...
		public boolean hitZoomHandle(int x, int y) {
			return new Rectangle(finderBounds.x + finderBounds.width - 6, finderBounds.y + finderBounds.height - 6, 8,
					8).contains(x, y);
		}

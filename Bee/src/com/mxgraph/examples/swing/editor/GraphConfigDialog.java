		...
		if ((graphType2 == GraphType.NULL) || (graphType2 == GraphType.SIMPLE_RANDOM_TREE)) {
			...
			applyButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					graph.removeCells();
					mxGraphGenerator generator = new mxGraphGenerator(
							mxGraphGenerator.getGeneratorFunction(graph, weighted, minWeightParam, maxWeightParam),
							new mxDoubleValCostFunction());
					Map<String, Object> props = new HashMap<String, Object>();
			...
			setResizable(false);
			// setLocationRelativeTo(parent);
		} else if ((graphType2 == GraphType.WHEEL) || (graphType2 == GraphType.STAR)
			...
			applyButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					generator.getSimpleRandomGraph(aGraph, nodeCount, edgeCount, allowSelfLoops, allowMultipleEdges,
							forceConnected);
			...
			setResizable(false);
			// setLocationRelativeTo(parent);
		} else if ((graphType2 == GraphType.BFS_DIR) || (graphType2 == GraphType.DFS_DIR)
				|| (graphType2 == GraphType.BFS_UNDIR) || (graphType2 == GraphType.DFS_UNDIR)
				|| (graphType2 == GraphType.MAKE_TREE_DIRECTED) || (graphType2 == GraphType.INDEGREE)
				|| (graphType2 == GraphType.OUTDEGREE) || (graphType2 == GraphType.IS_CUT_VERTEX)) {
			JPanel panel = new JPanel(new GridLayout(1, 2, 4, 4));
			...
			applyButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (startVertex == null) {
						...
						System.out.println("Indegree of " + aGraph.getGraph().getModel().getValue(startVertex) + " is "
								+ indegree);
					} else if (graphType2 == GraphType.OUTDEGREE) {
						int outdegree = mxGraphStructure.outdegree(aGraph, startVertex);
						System.out.println("Outdegree of " + aGraph.getGraph().getModel().getValue(startVertex) + " is "
								+ outdegree);
					} else if (graphType2 == GraphType.IS_CUT_VERTEX) {
						boolean isCutVertex = mxGraphStructure.isCutVertex(aGraph, startVertex);

						if (isCutVertex) {
							System.out.println("Vertex " + aGraph.getGraph().getModel().getValue(startVertex)
									+ " is a cut vertex.");
						} else {
							System.out.println("Vertex " + aGraph.getGraph().getModel().getValue(startVertex)
									+ " is not a cut vertex.");
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (graphType2 == GraphType.BELLMAN_FORD) {
						try {
							...
							System.out.print(
									"Distances from " + costFunction.getCost(view.getState(startVertex)) + " to [ ");

							for (int i = 0; i < vertexNum; i++) {
								System.out.print(i + ":"
										+ Math.round((Double) distanceMap.get(vertices[i]) * 100.0) / 100.0 + " ");
							}
							...
							for (int i = 0; i < vertexNum; i++) {
								System.out.print(i + ":"
										+ costFunction.getCost(view.getState(parentMap.get(vertices[i]))) + " ");
							}
							...
							if ((Double) distanceMap.get(endVertex) != Double.MAX_VALUE) {
								System.out.println("The shortest distance from vertex "
										+ costFunction.getCost(view.getState(startVertex)) + " to vertex "
										+ (Double) costFunction.getCost(view.getState(endVertex)) + " is: "

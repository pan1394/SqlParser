...
public class mxGraphicsCanvas2D implements mxICanvas2D {
	...
	/**
	 * Specifies the additional pixels when computing the text width for HTML
	 * labels. Default is 5.
	 */
	...
	/**
	 * Unit to be used for HTML labels. Default is "pt". If you units within HTML
	 * labels are used, this should match those units to produce a consistent
	 * output. If the value is "px", then HTML_SCALE should be changed the match the
	 * ratio between px units for rendering HTML and the units used for rendering
	 * other graphics elements. This value is 0.6 on Linux and 0.75 on all other
	 * platforms.
	 */
	...
	public void rect(double x, double y, double w, double h) {
		...
		currentPath.append(new Rectangle2D.Double((state.dx + x) * state.scale, (state.dy + y) * state.scale,
				w * state.scale, h * state.scale), false);
	}
	...
	public void ellipse(double x, double y, double w, double h) {
		currentPath = new GeneralPath();
		currentPath.append(new Ellipse2D.Double((state.dx + x) * state.scale, (state.dy + y) * state.scale,
				w * state.scale, h * state.scale), false);
	}
	 ...
	 */
	protected String createHtmlDocument(String text, String align, String valign, int w, int h, boolean wrap,
			String overflow, boolean clip) {
		...
		css.append("line-height:" + ((mxConstants.ABSOLUTE_LINE_HEIGHT)
				? Math.round(state.fontSize * mxConstants.LINE_HEIGHT) + " " + HTML_UNIT
	 ...
	 */
	protected void htmlText(double x, double y, double w, double h, String str, String align, String valign,
		...
		if (textRenderer != null && rendererPane != null) {
			...
			str = createHtmlDocument(str, align, valign, (widthFill || fill) ? (int) Math.round(w) : 0,
			...
			rendererPane.paintComponent(state.g, textRenderer, rendererPane, (int) Math.round(x / HTML_SCALE),
					(int) Math.round(y / HTML_SCALE), (int) Math.round(w / HTML_SCALE),
	 ...
	 * implementation returns the first entry for comma-separated lists of entries.
	 */
	...
	protected void updateStroke() {
		...
		if (lastStroke == null || lastStrokeWidth != sw || lastCap != cap || lastJoin != join
				|| lastMiterLimit != miterlimit || lastDashed != state.dashed

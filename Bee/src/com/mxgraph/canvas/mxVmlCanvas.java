...
public class mxVmlCanvas extends mxBasicCanvas {
	...
	/**
	 * Constructs a new VML canvas for the specified bounds, scale and background
	 * color.
	 */
		...
		if (state.getAbsolutePointCount() > 1) {
			...
			if (start != null || end != null) {
				if (start != null) {
					...
					double startSize = mxUtils.getFloat(style, mxConstants.STYLE_STARTSIZE,
							mxConstants.DEFAULT_MARKERSIZE) * scale;
				...
				if (end != null) {
					...
					double endSize = mxUtils.getFloat(style, mxConstants.STYLE_ENDSIZE, mxConstants.DEFAULT_MARKERSIZE)
							* scale;
			...
			if (!mxUtils.getString(style, mxConstants.STYLE_SHAPE, "").equals(mxConstants.SHAPE_SWIMLANE)) {
				...
				int start = (int) Math.round(
						mxUtils.getInt(style, mxConstants.STYLE_STARTSIZE, mxConstants.DEFAULT_STARTSIZE) * scale);
	...
	public Element drawShape(int x, int y, int w, int h, Map<String, Object> style) {
		...
		if (shape.equals(mxConstants.SHAPE_IMAGE)) {
			...
			String points = "ar 0 0 " + w + " " + h + " 0 " + (h / 2) + " " + (w / 2) + " " + (h / 2) + " e ar " + inset
					+ " " + inset + " " + (w - inset) + " " + (h - inset) + " 0 " + (h / 2) + " " + (w / 2) + " "
					+ (h / 2);
			...
			String points = "m " + (w / 2) + " 0 l " + w + " " + (h / 2) + " l " + (w / 2) + " " + h + " l 0 "
					+ (h / 2);
			...
			if (direction.equals(mxConstants.DIRECTION_NORTH) || direction.equals(mxConstants.DIRECTION_SOUTH)) {
				points = "m " + (int) (0.5 * w) + " 0 l " + w + " " + (int) (0.25 * h) + " l " + w + " "
						+ (int) (0.75 * h) + " l " + (int) (0.5 * w) + " " + h + " l 0 " + (int) (0.75 * h) + " l 0 "
						+ (int) (0.25 * h);
			} else {
				points = "m " + (int) (0.25 * w) + " 0 l " + (int) (0.75 * w) + " 0 l " + w + " " + (int) (0.5 * h)
						+ " l " + (int) (0.75 * w) + " " + h + " l " + (int) (0.25 * w) + " " + h + " l 0 "
						+ (int) (0.5 * h);
			}
			...
			String points = "m " + (int) (0.25 * w) + " " + (int) (0.25 * h) + " c " + (int) (0.05 * w) + " "
					+ (int) (0.25 * h) + " 0 " + (int) (0.5 * h) + " " + (int) (0.16 * w) + " " + (int) (0.55 * h)
					+ " c 0 " + (int) (0.66 * h) + " " + (int) (0.18 * w) + " " + (int) (0.9 * h) + " "
					+ (int) (0.31 * w) + " " + (int) (0.8 * h) + " c " + (int) (0.4 * w) + " " + (h) + " "
					+ (int) (0.7 * w) + " " + (h) + " " + (int) (0.8 * w) + " " + (int) (0.8 * h) + " c " + (w) + " "
					+ (int) (0.8 * h) + " " + (w) + " " + (int) (0.6 * h) + " " + (int) (0.875 * w) + " "
					+ (int) (0.5 * h) + " c " + (w) + " " + (int) (0.3 * h) + " " + (int) (0.8 * w) + " "
					+ (int) (0.1 * h) + " " + (int) (0.625 * w) + " " + (int) (0.2 * h) + " c " + (int) (0.5 * w) + " "
					+ (int) (0.05 * h) + " " + (int) (0.3 * w) + " " + (int) (0.05 * h) + " " + (int) (0.25 * w) + " "
					+ (int) (0.25 * h);
			...
			double width3 = w / 3;
			String points = "m 0 " + (h) + " C 0 " + (3 * h / 5) + " 0 " + (2 * h / 5) + " " + (w / 2) + " "
					+ (2 * h / 5) + " c " + (int) (w / 2 - width3) + " " + (2 * h / 5) + " " + (int) (w / 2 - width3)
					+ " 0 " + (w / 2) + " 0 c " + (int) (w / 2 + width3) + " 0 " + (int) (w / 2 + width3) + " "
					+ (2 * h / 5) + " " + (w / 2) + " " + (2 * h / 5) + " c " + (w) + " " + (2 * h / 5) + " " + (w)
					+ " " + (3 * h / 5) + " " + (w) + " " + (h);
			...
			String points = "m 0 " + (int) (dy) + " C 0 " + (int) (dy / 3) + " " + (w) + " " + (int) (dy / 3) + " "
					+ (w) + " " + (int) (dy) + " L " + (w) + " " + (int) (h - dy) + " C " + (w) + " "
					+ (int) (h + dy / 3) + " 0 " + (int) (h + dy / 3) + " 0 " + (int) (h - dy) + " x e" + " m 0 "
					+ (int) (dy) + " C 0 " + (int) (2 * dy) + " " + (w) + " " + (int) (2 * dy) + " " + (w) + " "
					+ (int) (dy);
		...
		String s = "position:absolute;left:" + String.valueOf(x) + "px;top:" + String.valueOf(y) + "px;width:"
				+ String.valueOf(w) + "px;height:" + String.valueOf(h) + "px;";
	...
	/**
	 * Draws the given lines as segments between all points of the given list of
	 * mxPoints.
	 * 
	 * @param pts
	...
	public Element drawLine(List<mxPoint> pts, Map<String, Object> style) {
		...
		if (strokeColor != null && strokeWidth > 0) {
			...
			String s = "position:absolute;" + "left:" + String.valueOf(r.x) + "px;" + "top:" + String.valueOf(r.y)
					+ "px;" + "width:" + String.valueOf(r.width) + "px;" + "height:" + String.valueOf(r.height) + "px;";
			elem.setAttribute("style", s);

...
/**
 * The <code>ExtendedPathIterator</code> class represents a geometric path
...
public interface ExtendedPathIterator {
	 ...
	 * curve is interpolated by solving the parametric control equation in the range
	 * (t=[0..1]) using the most recently specified (current) point (CP), the first
	 * control point (P1), and the final interpolated control point (P2).
	 */
	 ...
	 * curve is interpolated by solving the parametric control equation in the range
	 * (t=[0..1]) using the most recently specified (current) point (CP), the first
	 * control point (P1), the second control point (P2), and the final interpolated
	 * control point (P3).
	 */
	int SEG_CUBICTO = PathIterator.SEG_CUBICTO;

	/**
	 * The segment type constant for an elliptical arc. This consists of Seven
	 * values [rx, ry, angle, largeArcFlag, sweepFlag, x, y]. rx, ry are the radious
	 * of the ellipse. angle is angle of the x axis of the ellipse. largeArcFlag is
	 * zero if the smaller of the two arcs are to be used. sweepFlag is zero if the
	 * 'left' branch is taken one otherwise. x and y are the destination for the
	 * ellipse.
	 */
	int SEG_ARCTO = 4321;

	/**
	 * The winding rule constant for specifying an even-odd rule for determining the
	 * interior of a path. The even-odd rule specifies that a point lies inside the
	 * path if a ray drawn in any direction from that point to infinity is crossed
	 * by path segments an odd number of times.
	 */

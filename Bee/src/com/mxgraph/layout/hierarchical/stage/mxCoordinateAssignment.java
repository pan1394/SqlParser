...
public class mxCoordinateAssignment implements mxHierarchicalLayoutStage {
	 ...
	 * for that edge where it connects to the source and target vertices. Note this
	 * should technically be a WeakHashMap, but since JS does not have an
	 * equivalent, housekeeping must be performed before using. i.e. check all edges
	 * are still in the model and clear the values. Note that the y co-ord is the
	 * offset of the jetty, not the absolute point
	 */
	protected Map<mxGraphHierarchyEdge, double[]> jettyPositions = new HashMap<mxGraphHierarchyEdge, double[]>();

	/**
	 * The position of the root ( start ) node(s) relative to the rest of the laid
	 * out graph
	 */
	...
	private void minNode(mxGraphHierarchyModel model) {
		...
		while (!nodeList.isEmpty() && count <= maxTries) {
			...
			if (numConnectedNeighbours > 0) {
				cellMedian = (medianNextLevel * numNextLayerConnected + medianPreviousLevel * numPreviousLayerConnected)
	 ...
	 *            an internal model of the hierarchical layout
	 * @param nextRankValue
	 *            the layer number whose connected cels are to be laid out relative
	 *            to
	 */
				...
				if (weightedValue != null) {
					...
					if (weightedValue.visited) {
						...
						leftLimit = leftCell.getGeneralPurposeVariable(rankValue) + leftCell.width / 2.0
								+ intraCellSpacing + leftBuffer + cell.width / 2.0;
						j = -1;
					} else {
				...
				if (weightedValue != null) {
					...
					if (weightedValue.visited) {
						...
						rightLimit = rightCell.getGeneralPurposeVariable(rankValue) - rightCell.width / 2.0
								- intraCellSpacing - rightBuffer - cell.width / 2.0;
	 ...
	 */
	private int calculatedWeightedValue(mxGraphAbstractHierarchyCell currentCell,
	...
	/**
	 * Sets up the layout in an initial positioning. The ranks are all centered as
	 * much as possible along the middle vertex in each rank. The other cells are
	 * then placed as close as possible on either side.
	 * 
	 * @param facade
	 ...
	 * together as their size and buffering permits. This method works on just the
	 * specified rank.
	 * 
	 * @param rankValue
	...
	/**
	 * Determines whether or not a node may be moved to the specified x position on
	 * the specified rank
	 * 
	 * @param model
	 *            the layout model
	 ...
	 */
	protected boolean repositionValid(mxGraphHierarchyModel model, mxGraphAbstractHierarchyCell cell, int rank,
			double position) {
	 ...
	 * implementation adjusts the group to just fit around the children with a
	 * padding.
	 */
				...
				if (cell.isVertex()) {
					...
					for (int k = 0; k < 2; k++) {
						...
						currentCells = (cell.getNextLayerConnectedCells(i))
								.toArray(new mxGraphAbstractHierarchyCell[cell.getNextLayerConnectedCells(i).size()]);
	 ...
	 * of their connected edges. Does not violate (x.compareTo(y)==0) ==
	 * (x.equals(y))
	 */
	...
	/**
	 * Utility class that stores a collection of vertices and edge points within a
	 * certain area. This area includes the buffer lengths of cells.
	 */

...
public class mxCompactTreeLayout extends mxGraphLayout {
	...
	/**
	 * A set of the parents that need updating based on children process as part of
	 * the layout
	 */
	 ...
	 * 
	 * If the parent has any connected edges, then it is used as the root of the
	 * tree. Else, <mxGraph.findTreeRoots> will be used to find a suitable root node
		...
		if (cell != null && !visited.contains(cell) && !isVertexIgnored(cell)) {
			...
			for (int i = 0; i < out.length; i++) {
				...
				if (!isEdgeIgnored(edge)) {
					...
					Object target = (state != null) ? state.getVisibleTerminal(invert)
							: view.getVisibleTerminal(edge, invert);

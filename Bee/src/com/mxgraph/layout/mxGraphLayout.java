...
public abstract class mxGraphLayout implements mxIGraphLayout {
	...
	/**
	 * Returns the constraint for the given key and cell. This implementation always
	 ...
	 * source arguments are used to return inbound and outgoing routing- constraints
	 * for the given edge and vertex. This implementation always returns the value
	 * for the given key in the style of the given cell.
	 * 
	 * @param key
	 *            Key of the constraint to be returned.
	 * @param cell
	 ...
	 * @param source
	 *            Optional boolean that specifies if the connection is incoming or
	 *            outgoing. Default is false.
	 */
	...
	public boolean isEdgeIgnored(Object edge) {
		...
		return !model.isEdge(edge) || !graph.isCellVisible(edge) || model.getTerminal(edge, true) == null
	...
	/**
	 * Sets the control points of the given edge to the given list of mxPoints. Set
	 * the points to null to remove all existing points for an edge.
	 */
	...
	public mxRectangle getVertexBounds(Object vertex) {
		...
		if (useBoundingBox) {
			...
			if (state != null) {
				...
				geo = new mxRectangle(geo.getX() + dx0, geo.getY() + dy0, geo.getWidth() - dx0 + dx1,
	 ...
	 * the new location is not equal to the existing location, otherwise the
	 * geometry is not replaced with an updated instance. The new or old bounds are
	 * returned (including overlapping labels).
	 * 
	 * Parameters:
	 * 
	 * cell - <mxCell> whose geometry is to be set. x - Integer that defines the
	 * x-coordinate of the new location. y - Integer that defines the y-coordinate
	 * of the new location.
	 */
	 ...
	 * 
	 * arrangeGroups(graph, mxUtils.sortCells(Arrays.asList( new Object[] { v1, v3
	 * }), true).toArray(), 10);
	 * 
	 * @param groups

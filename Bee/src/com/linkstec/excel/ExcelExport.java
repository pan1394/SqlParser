...
//			if (target != null) {
//
//				Sheet sqlSheet = this.createSheet(workBook, "SQL仕様書", "SQL仕様書(" + target.getLogicName() + ")");
...
//			}
//
//			Sheet history = workBook.getSheet("更新履歴");
//			history.getRow(1).getCell(6).setCellValue(batchId);
//			history.getRow(1).getCell(10).setCellValue(batchName);

			Sheet sheet = this.createSheet(workBook, "モジュールクラス", "モジュールクラス(" + mainSheetName + ")");
			new ExcelProcess(project, mainClass, sheet, style, doInvoker, doStatic);

			Sheet dto = workBook.getSheet("Dto設計");
			new ExcelDto(dto, mainClass, project, style, models);
			...
			String filename = "(" + name + ")バッチ内部設計.xlsx";

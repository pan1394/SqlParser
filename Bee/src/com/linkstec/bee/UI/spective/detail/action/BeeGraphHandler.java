...
public class BeeGraphHandler extends mxGraphHandler {
	...
	private BasicNode targetScope;

	public BeeGraphHandler(mxGraphComponent graphComponent) {
	...
	private boolean makeMarker(DropTargetDragEvent e) {
		...
		if (this.dragCells != null) {
			...
			if (!(this.dragCells[0] instanceof BasicNode)) {
				...
				source = (BasicNode) this.dragCells[0];
			}

			this.targetScope = null;
			String scopeID = (String) source.getUserAttribute("SCOPE_ID");
			if (scopeID != null) {
				BeeModel model = (BeeModel) graphComponent.getGraph().getModel();
				mxICell scope = model.getPossibleCellById(scopeID);
				if (scope instanceof BasicNode) {
					this.targetScope = (BasicNode) scope;
				}
			}
				...
				if (state != null) {
					...
					if (object instanceof BLockNode) {
						...
						if (cells != null && cells.length > 0) {
							position.setIndex(getIndex((mxICell) cells[cells.length - 1]));
							Object obj = cells[cells.length - 1];

							if (obj instanceof BasicNode) {
								BasicNode b = (BasicNode) obj;
								if (b.getParent() instanceof BLockNode) {
									if (this.inScope((BasicNode) b.getParent())) {
										mxRectangle box = graph.getGraph().getBoundingBox(obj);
										dragDropMarker = new Rectangle((int) box.getX(), (int) box.getY() + (int) box.getHeight() + hmargin / 2, (int) (box.getWidth()), hheight);
									}
									// }
								} else {
									if (this.inScope(b)) {
										if (b.isDropTarget(source)) {
											mxRectangle box = graph.getGraph().getBoundingBox(obj);
											dragDropMarker = new Rectangle((int) box.getX(), (int) box.getY() + (int) box.getHeight() + hmargin / 2, (int) (box.getWidth()), hheight);
										}
									}
							...
							if (this.inScope(node)) {
								if (node.isDropTarget(source)) {
									boolean firstExist = false;
									if (node.getChildCount() > 0) {
										mxICell first = node.getChildAt(0);
										if (first instanceof BasicNode) {
											BasicNode f = (BasicNode) first;
											if (f.getUserAttribute("BodyTitle") != null) {
												firstExist = true;
												mxRectangle box = graph.getGraph().getBoundingBox(f);
												dragDropMarker = new Rectangle((int) box.getX(), (int) box.getY() + (int) box.getHeight() + hmargin / 2, (int) (box.getWidth()), hheight);
												position.setIndex(1);
											}
										}
									...
									if (!firstExist) {
										position.setIndex(0);
										mxRectangle box = graph.getGraph().getBoundingBox(node);
										dragDropMarker = new Rectangle((int) box.getX() + hmargin, (int) box.getY() + hmargin / 2, (int) (box.getWidth() - hmargin * 2), hheight);
									}
								}
							}
						}
			...
			if (!blockLine) {
				if (draggingOver == null && this.targetScope == null) {
					...
					if (cells != null && cells.length > 0) {
						...
						if (!hasHeader) {
							position.setIndex(0);
							dragDropMarker = new Rectangle((int) (BeeConstants.PAGE_SPACING_LEFT * scale), hmargin / 2, (int) (BeeConstants.SEGMENT_EDITOR_DEFAULT_WIDTH * scale), hheight);
						}
					}
				}
			}
	...
	private boolean inScope(BasicNode node) {
		if (this.targetScope != null) {
			while (node != null) {
	...
	public BasicNode getTargetScope() {
		return targetScope;
	}

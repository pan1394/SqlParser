...
public class BeeLoigcEditor implements MouseMotionListener, MouseListener {
	...
	public BeeLoigcEditor(BeeGraphSheet sheet) {
		this.sheet = sheet;
	...
	public void paint(mxCellState state, BeeCanvas canvas) {
	...
	private void drawCell(Graphics2D g, BasicNode node, mxCellState state) {
	...
	private void makeTargetInfo(mxCellState state, Graphics2D g) {
		...
		if (c instanceof BasicNode) {
			BasicNode n = (BasicNode) c;
			action = n.getAction();
		} else if (c instanceof ModifierNode) {
			ModifierNode m = (ModifierNode) c;
			action = m.getAction();
			rect.x = (int) (box.getX()) + 1;
			// rect.y = (int) (box.getY()) + (int) box.getHeight() - 2;
					...
					for (ActionComponent a : comps) {
						if (a.isMouseOver()) {
							...
							if (b.contains(e.getPoint())) {
								...
								firstAction = null;
								this.target = null;
								LayoutUtils.RelayoutAll(this.sheet);
								break;
							}
						} else {
							if (firstAction == null) {
	...
	public void paintHandler(Graphics g) {
		...
		int right = sheet.getBorder().getBorderInsets(sheet).right;

...
import com.linkstec.bee.UI.spective.basic.logic.model.var.ListGetLogic;
...
public class BListGetNode extends BNode implements ILogicCell {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4301030782833490217L;
	private ListGetLogic logic;
	...
	public void setLogic(ListGetLogic logic) {
		this.logic = logic;
	}
	...
	public void childAdded(BNode node, BasicLogicSheet sheet) {
		if (node instanceof BDetailNodeWrapper) {
			...
			if (l instanceof JudgeLogic) {
				JudgeLogic judge = (JudgeLogic) l;
				BValuable ex = judge.getExpression(null);
				if (ex instanceof mxICell) {
					cellAdded((mxICell) ex);
				}
			}
		}

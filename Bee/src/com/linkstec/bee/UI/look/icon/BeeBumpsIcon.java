...
public class BeeBumpsIcon implements Icon {
	 ...
	 */
	public BeeBumpsIcon(int width, int height, Color newTopColor, Color newShadowColor, Color newBackColor) {
	...
	private void createImage() {
		if (gc != null) {
			image = gc.createCompatibleImage(IMAGE_SIZE, IMAGE_SIZE,
					(backColor != BeeBumpsIcon.ALPHA) ? Transparency.OPAQUE : Transparency.BITMASK);
		} else {
			int cmap[] = { backColor.getRGB(), topColor.getRGB(), shadowColor.getRGB() };
			IndexColorModel icm = new IndexColorModel(8, 3, cmap, 0, false, (backColor == BeeBumpsIcon.ALPHA) ? 0 : -1,

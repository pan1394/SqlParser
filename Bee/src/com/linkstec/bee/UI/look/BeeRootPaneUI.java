...
public class BeeRootPaneUI extends BasicRootPaneUI {
	/**
	...
	public static ComponentUI createUI(JComponent c) {
		return new BeeRootPaneUI();
	}

	/**
	 * Invokes supers implementation of <code>installUI</code> to install the
	 * necessary state onto the passed in <code>JRootPane</code> to render the metal
	 * look and feel implementation of <code>RootPaneUI</code>. If the
	 * <code>windowDecorationStyle</code> property of the <code>JRootPane</code> is
	 * other than <code>JRootPane.NONE</code>, this will add a custom
	 * <code>Component</code> to render the widgets to <code>JRootPane</code>, as
	 * well as installing a custom <code>Border</code> and
	 ...
	 * reset the <code>LayoutManager</code> of the <code>JRootPane</code>. If a
	 * <code>Component</code> has been added to the <code>JRootPane</code> to render
	 * the window decoration style, this method will remove it. Similarly, this will
	 * revert the Border and LayoutManager of the <code>JRootPane</code> to what it
	 * was before <code>installUI</code> was invoked.
	 *
	 * @param c
	 ...
	 * This is ONLY invoked if the <code>JRootPane</code> has a decoration style
	 * other than <code>JRootPane.NONE</code>.
	 */
	 ...
	 * installed.
	 * <p>
	 * NOTE: This may be called if you haven't installed client decorations yet (ie
	 * before <code>installClientDecorations</code> has been invoked).
	 */
	...
	private JComponent createTitlePane(JRootPane root) {
		Container c = root.getTopLevelAncestor();
		if (c instanceof BeePopUI) {
			return null;
		}
		return new BeeTitlePane(root, this);
	}
	...
	/**
	 * Returns the <code>JComponent</code> rendering the title pane. If this returns
	 * null, it implies there is no need to render window decorations.
	 *
	...
	private static class MetalRootLayout implements LayoutManager2 {
		...
		public Dimension preferredLayoutSize(Container parent) {
			...
			if (root.getWindowDecorationStyle() != JRootPane.NONE && (root.getUI() instanceof BeeRootPaneUI)) {
				JComponent titlePane = ((BeeRootPaneUI) root.getUI()).getTitlePane();
				if (titlePane != null) {
